...
bool NetworkUtil::bind(SOCKET sockfd,const std::string&ip,uint16_t port)
{
    ...
    {
        FERR("bind {} to {} {} failed{}",sockfd,ip.c_str(),port,strerror(io::platform::getErrno()));
        return false;
    }
    return true;
}
...
bool NetworkUtil::connect(SOCKET sockfd,std::string ip,uint16_t port,uint32_t time_out_ms)
{
    ...
    if(!isConnected){
        FERR("connect {} to {} {} {}ms failed{}",sockfd,ip.c_str(),port,time_out_ms,strerror(io::platform::getErrno()));
    }
    return isConnected;
}
...
void NetworkUtil::set_reuse_addr(SOCKET fd)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == -1)
        FERR("setsockopt SO_REUSEADDR fd {}: {}", fd, strerror(platform::getErrno()));
}
...
NetworkUtil::sock_set_v6only(SOCKET fd)
{
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on)) == -1)
        FERR("setsockopt IPV6_V6ONLY: {}", strerror(errno));
#endif
}
...
uint16_t NetworkUtil::get_local_port(SOCKET sockfd)
{
    ...
    FERR("getsockname failed {}",strerror(platform::getErrno()));
    return 0;
}
...
std::string NetworkUtil::get_host_info(sockaddr_in *addr)
{
    ...
    {
        FERR("inet_ntop failed {}",strerror(platform::getErrno()));
    }
    else {
        ret=buf;
    }
