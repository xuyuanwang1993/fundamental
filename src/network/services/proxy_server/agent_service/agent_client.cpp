...
bool AgentClient::CancelRequest(AgentClientToken token) {
    ...
    do {
        if (iter == sessions_.end()) break;
        auto ptr = iter->second.lock();
        if (!ptr) { // maybe some bugs existed if the program reach this case
...
template <>
void AgentClient::AgentHandler::PackageProxyFrame<AgentUpdateContext>(AgentRequestFrame& frame,
                                                                      const AgentUpdateContext& context) {
    using SizeType       = decltype(frame.payload)::SizeType;
    SizeType payloadSize = sizeof(SizeType) * 3 + context.request.data.GetSize() + context.request.id.GetSize() +
                           context.request.section.GetSize() + sizeof(context.request.op);
...
template <>
void AgentClient::AgentHandler::PackageProxyFrame<AgentSniffContext>(AgentRequestFrame& frame,
                                                                     const AgentSniffContext& context) {
}

template <>
void AgentClient::AgentHandler::PackageProxyFrame<AgentQueryContext>(AgentRequestFrame& frame,
                                                                     const AgentQueryContext& context) {
    using SizeType       = decltype(frame.payload)::SizeType;
    SizeType payloadSize = sizeof(SizeType) * 2 + context.request.id.GetSize() + context.request.section.GetSize() +
...
AgentClientToken AgentClient::AgentHandler::HandleAgentRequest(AgentClient* client, const ContextType& context) {
    ...
    PackageProxyFrame(requestFrame, context);
    {
        ...
        SizeType payloadSize = sizeof(SizeType) * 2 + requestFrame.cmd.GetSize() + requestFrame.payload.GetSize() +
...
AgentClient::AgentSession::AgentSession(const std::string& host, const std::string& service,
                                        const AgentFinishedCallback& cb, ProxyFrame&& frame) :
host(host),
service(service), frame(std::move(frame)), socket_(io_context_pool::Instance().get_io_context()),
resolver(socket_.get_executor()), finishCb(cb) {
}
    ...
    for (;;) {
        ...
        case AgentDnsResolving: {
            if (status_.compare_exchange_strong(old_Value, AgentCancelled)) {
                asio::post(resolver.get_executor(), [ref, this]() { resolver.cancel(); });
                return;
        ...
        case AgentRecving: {
            ...
            if (status_.compare_exchange_strong(old_Value, AgentCancelled)) {
                asio::post(socket_.get_executor(), [ref, this]() { socket_.close(); });
                return;
            }
        } break;
    ...
    asio::async_read(
        ...
        [this, self = shared_from_this()](std::error_code ec, std::size_t bytes_transferred) {
            do {
                ...
                if (!ProxyRequestHandler::DecodeHeader(headerBuffer.data(), headerBuffer.size(), responseFrame)) {
...
void AgentClient::AgentSession::HandleResponse() {
    ...
    try {
        ...
        if (finishCb) finishCb(true, reinterpret_cast<AgentClientToken>(this), std::move(tmp));
...
void AgentClient::AgentSession::HandleFailed(asio::error_code ec) {
    FDEBUG("request failed for {} {} final status {} ec:{}", host, service, static_cast<std::int32_t>(status_.load()),
           ec.message());
    if (status_ != AgentCancelled) status_.exchange(AgentFinishedFailed);
    if (finishCb) {
        finishCb(false, reinterpret_cast<AgentClientToken>(this), AgentResponse {});
    }
}
