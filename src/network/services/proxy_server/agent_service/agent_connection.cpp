    ...
    static void ProcessUpdate(AgentRequestFrame& request, std::shared_ptr<AgentConnection> connection) {
        ...
        try {
            ...
            AgentStorage::Instance().UpdateAgentInfo(requestData.id, requestData.section, std::move(requestData.data),
                                                     requestData.op);
        } catch (const std::exception&) { // close connection
            return;
        }
        ...
        if (ipInfo == AgentSniffRequest::kLoopbackIp) {
            ...
            if (iter != networkInfo.end()) {
                ...
                for (auto& item : networkInfo) {
                    if (!item.second.isLoopback) ipInfo = item.second.ipv4;
                }
            }
        }
        AgentSniffResponse responseData;
        responseData.host    = ipInfo;
        SizeType payloadSize = responseData.host.GetSize() + sizeof(responseData.host.GetSize());
    ...
    static void ProcessQuery(AgentRequestFrame& request, std::shared_ptr<AgentConnection> connection) {
        ...
        auto delay_process_func = [=]() -> bool {
            ...
            SizeType payloadSize =
                sizeof(responseData->timestamp) + responseData->data.GetSize() + sizeof(responseData->data.GetSize());
            response->payload.Reallocate(payloadSize);
            Fundamental::BufferWriter<SizeType> writer;
            writer.SetBuffer(response->payload.GetData(), response->payload.GetSize());
...
static Fundamental::ScopeGuard s_register(nullptr, []() {
    ...
    AgentConnection::cmd_handlers.emplace(std::string(AgentUpdateRequest::kCmd),
                                          AgentConnection::details::ProcessUpdate);
...
AgentConnection::~AgentConnection() {
    ...
    if (delay_timer) delay_timer->cancel();
}
...
void AgentConnection::HandleResponse(AgentResponseFrame& response) {
    ...
    std::size_t payloadSize = 3 * sizeof(response.cmd.GetSize()) + response.cmd.GetSize() + response.msg.GetSize() +
...
void AgentConnection::HandleDelayRequest(const std::function<bool()>& check_func) {
    ...
    asio::async_read(
        socket_, asio::buffer(&delay_buf, 1), [this, self = shared_from_this()](std::error_code ec, std::size_t) {
            if (ec) {
