...
bool ProxyRequestHandler::DecodeHeader(const std::uint8_t* data, std::size_t len, ProxyFrame& dstFrame) {
    do {
        if (len < ProxyFrame::kHeaderSize) break;
        std::size_t offset = 0;
        // docode fixed 2bytes
        std::memcpy(&dstFrame.fixed, data, 2);
        dstFrame.fixed = le16toh(dstFrame.fixed);
        if (dstFrame.fixed != ProxyFrame::kFixed) break;
        offset += 2;
        ...
        if (dstFrame.version > ProxyFrame::kVersion) break;
        ++offset;
        // decode op
        ...
        if (dstFrame.sizeStorage > ProxyFrame::kMaxFrameSize) break;
...
void ProxyRequestHandler::UpgradeProtocal(Connection&& connection) {
    ...
    if (iter != handlers_.end()) {
        ...
        if (newConnection) newConnection->SetUp();
    } else {
        FWARN("unsupported proxy op {} for version {}", ProxyFrame::kVersion, connection.requestFrame.op);
    }
}
...
void ProxyRequestHandler::RegisterProtocal(std::uint8_t opCode, ProtocalHandler handler) {
    ...
    if (handler) handlers_[opCode] = handler;
}

ProxeServiceBase::ProxeServiceBase(asio::ip::tcp::socket&& socket, ProxyFrame&& frame) :
socket_(std::forward<asio::ip::tcp::socket>(socket)), frame(std::forward<ProxyFrame>(frame)) {
}
