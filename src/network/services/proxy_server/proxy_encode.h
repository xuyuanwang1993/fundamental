...
inline void proxy_encode_request(proxy_encode_input& input, proxy_encode_output& output) {
    uint64_t payload_len = sizeof(uint64_t) * 3 + input.fieldLen + input.serviceLen + input.tokenLen;
    output.bufLen        = sizeof(uint16_t) + 1 + 1 + 1 + sizeof(uint32_t) + sizeof(uint64_t) + payload_len;
