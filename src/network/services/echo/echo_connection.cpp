...
connection::connection(asio::ip::tcp::socket socket, EchoRequestHandler& handler) :
...
void connection::do_read() {
    auto size =
        msgContext_.IsWaitSize() ? EchoMsg::kHeaderSize : msgContext_.msg.msg.size() - msgContext_.currentOffset;
    if (size > kPerReadMaxBytes) size = kPerReadMaxBytes;
    asio::async_read(socket_,
                     msgContext_.IsWaitSize()
                         ? asio::buffer(&msgContext_.msg.header.data, size)
                         : asio::buffer(msgContext_.msg.msg.data() + msgContext_.currentOffset, size),
...
void EchoRequestHandler::Process(const MsgContext& req, EchoMsg& reply) {
    ...
    reply.TimeStamp() =
        htobe64((Fundamental::Timer::GetTimeNow<std::chrono::seconds, std::chrono::system_clock>() + 1200));
}
...
decltype(MsgContext::status) Paser::PaserRequest(MsgContext& msgContext, std::size_t dataLen) {
    if (0 == dataLen) return msgContext.status = MsgContext::PaserFailed;
    if (msgContext.IsWaitSize()) {
        if (dataLen != EchoMsg::kHeaderSize) return msgContext.status = MsgContext::PaserFailed;
        auto size = be32toh(msgContext.msg.header.v);
        if (size == EchoMsg::kTimeStampSize) return msgContext.status = MsgContext::RequestExit;
        ...
        if (msgContext.currentOffset == msgContext.msg.msg.size()) return msgContext.status = MsgContext::PaserFinished;
        return msgContext.status = MsgContext::PaserNeedMoreData;
    }
}
