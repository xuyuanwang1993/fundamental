...
class echo_client {
public:
    echo_client(asio::io_context& io_context, const tcp::resolver::results_type& endpoints) :
...
private:
    ...
    void do_proxy_handshake() {
        ...
        asio::async_write(socket_, buf_slices_.front(), [this](std::error_code ec, std::size_t length) {
            if (!ec) {
                ...
                asio::async_read(socket_, asio::mutable_buffer(handshake, 2),
    ...
    void do_read_header() {
        asio::async_read(socket_, asio::buffer(msgContext_.msg.header.data, EchoMsg::kHeaderSize),
                         [this](std::error_code ec, std::size_t bytes_transferred) {
                             if (!ec) {
                                 ...
                                 default: handle_disconnected(ec); break;
                                 }
                             } else {
                                 handle_disconnected(ec);
                             }
    ...
    void do_read_body() {
        asio::async_read(socket_, asio::buffer(msgContext_.msg.msg.data(), msgContext_.msg.msg.size()),
                         [this](std::error_code ec, std::size_t bytes_transferred) {
                             if (!ec) {
    ...
    void do_write() {
        slice_data();
        socket_.async_write_some(buf_slices_.front(), [this](std::error_code ec, std::size_t length) {
            if (!ec) {
    ...
    void peek_slice_front(std::size_t length) {
        ...
        if (buf_slices_.empty()) write_msgs_.pop_front();
    }
