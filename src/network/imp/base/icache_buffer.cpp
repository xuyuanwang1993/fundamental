...
std::pair<const void *,uint32_t> IcacheBufferBase::popFrame()
{
    uint32_t ret_len=0;
    do{
...
bool IcacheBufferBase::appendCache(const void *data,uint32_t len)
{
    ...
    if(len+write_offset>cache_size)
    {
        ...
        if(data_size+len>max_cache_size){
            FERR("buffer overflow!max_size[{}] -> need [{}]",max_cache_size,data_size+len);
            return false;
        }
        ...
        {//尝试缩小所用内存
            new_size=data_size+protocal->getMaxFrameSize()*10;
        }//case 2
        else {
            new_size=cache_size+protocal->getMaxFrameSize()*10;
        }
