...
std::pair<std::shared_ptr<uint8_t>,uint32_t> ProtocalStream::packetFrame(const void *header, uint16_t header_len, const void * payload, uint32_t payload_len)
{
    ...
    uint64_t timestamp=platform::localToBe64(Fundamental::Timer::GetTimeNow<std::chrono::milliseconds>());
...
std::pair<std::shared_ptr<uint8_t>,uint32_t> ProtocalStream::encodeFrame(const std::list<std::pair<const void *,uint32_t>> &input_list)
{
    ...
    do{
        ...
        {
            FERR("false input for Pframe encode");
            break;
        }
...
std::pair<uint32_t/*frame start offset*/,uint32_t/*frame len*/> ProtocalStream::decodeFrame(const void *data,uint32_t len)
{
    ...
    do{
        ...
        uint64_t time_now=Fundamental::Timer::GetTimeNow<std::chrono::milliseconds>();
        ...
        {
            FDEBUG("version:{} {}",header.version,PROTOCAL_STREAM_VERSION);
            FDEBUG("reserved:{}",header.reserved);
            FDEBUG("need_size {}:{}",need_size,getMaxFrameSize());
            FDEBUG("time %lu:%lu:%lu:%lu",header.timestamp,time_now,(time_now-header.timestamp),PROTOCAL_STREAM_MAX_TIME_DIFF);
            FERR("header check failed!");
...
bool ProtocalStream::bufferToFrame(void *buf, uint32_t buf_len, StreamFrame * frame)
{
    ...
    do{
        ...
        if(size!=buf_len)
        {
            FERR("{} != {},size not match",size,buf_len);
            break;
        }
        ...
        {
            FERR("{} != {},check_code not match",check_code,frame->header.check_code);
            break;
        }
        ret=true;
