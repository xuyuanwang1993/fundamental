...
bool RttrMetaControlOption::HasInterSection(const RttrControlMetaDataType& data1, const RttrControlMetaDataType& data2)
{
    ...
    do
    {
        ...
        if (propExclueData.is_type<RttrControlMetaDataType>())
        {
            ...
            if (!propExclueDataRef.empty() && !excludeDatas.empty())
            {
                ...
                if (HasInterSection(propExclueDataRef, excludeDatas))
                    break;
            }
        }
        ...
        if (propIncludeData.is_type<RttrControlMetaDataType>())
        {
            ...
            {
                // check whether prop is excluded
                if (!HasInterSection(propIncludeDataRef, includeDatas))
                    break;
            }
        }
        return true;
