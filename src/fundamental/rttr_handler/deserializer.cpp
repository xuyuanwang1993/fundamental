...
void fromjson_recursively(instance obj, const Fundamental::json& json_object, const Fundamental::RttrDeserializeOption& option);
...
static void write_array_recursively(variant_sequential_view& view, const Fundamental::json& json_array_value, const Fundamental::RttrDeserializeOption& option)
{
...
variant extract_value(const Fundamental::json& json_value, const type& t, const Fundamental::RttrDeserializeOption& option)
{
...
static void write_associative_view_recursively(variant_associative_view& view, const Fundamental::json& json_array_value, const Fundamental::RttrDeserializeOption& option)
{
    for (size_t i = 0; i < json_array_value.size(); ++i)
    {
        auto& json_index_value = json_array_value[i];
...
void fromjson_recursively(instance obj2, const Fundamental::json& json_object, const Fundamental::RttrDeserializeOption& option)
{
...
static bool DfsSetProperties(rttr::variant& var, std::list<std::string>& properties, const std::string& value, const RttrDeserializeOption& option)
{
    type t = var.get_type();
    if (properties.empty())
    {
    ...
    if (ret)
        prop.set_value(objActual, prop_value);
    return ret;
};
...
bool set_property_by_json(rttr::instance obj, const std::string& propertyPath, const std::string& value, const RttrDeserializeOption& option)
{
    if (propertyPath.empty())
        return from_json(value, obj, option);
    auto splitFunction = [](
                             const std::string& input,
                             const std::string& sep) -> std::list<std::string> {
