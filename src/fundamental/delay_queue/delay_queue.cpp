...
{
    inline HandleType AddDelayTask(std::int64_t intervalMs, const TaskType& task, bool isSingle = false, bool autoManager = true)
    {
        if (intervalMs < 0 || !task)
            return DelayQueue::kInvalidHandle;
        if (stateCb)
            stateCb();
    ...
    {
        if (handle == DelayQueue::kInvalidHandle)
            return;
        if (stateCb)
            stateCb();
        std::scoped_lock<std::mutex> locker(dataMutex);
    ...
    inline bool ValidateInternal(HandleType handle)
    {
        if (handle == DelayQueue::kInvalidHandle)
            return false;
        if (taskStorage.find(Cast(handle)) == taskStorage.end())
            return false;
        return true;
    }
