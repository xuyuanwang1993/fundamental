...
inline std::vector<std::string> StringSplitIntoVector(
    const std::string& input, const std::vector<std::string>& delim, bool bCullEmpty = true)
{
...
inline bool StringEndWith(const std::string& s,
                          const std::string& potential_end)
{
...
{
    std::int32_t endIndex = static_cast<std::int32_t>(input.size()) - 1;
...
inline std::string StringReplace(
    const std::string& input,
