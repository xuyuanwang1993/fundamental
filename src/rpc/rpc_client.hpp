...
public:
    ...
    void enable_timeout_check(bool enable = true, std::size_t timeout_sec = 30) {
        if (enable) {
            reset_deadline_timer(timeout_sec);
        } else {
            deadline_.cancel();
        }
    }
            ...
            if (!ec) {
                if (has_connected_) {
                    if (b_wait_any_data) {
                        ...
                        FWARN("disconnect for timeout {} sec,we has not recv any data", timeout);
                        close();
                    } else {
