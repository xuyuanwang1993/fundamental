...
void Socks5Session::start(const void* probe_data, std::size_t probe_len) {
    ...
    try {
        ...
        FDEBUG("New Client Connection {}", convert::format_address(this->tcp_cli_endpoint));
        ...
        if (probe_len == kMaxProbeLen) {
            ...
            asio::async_read(this->socket, asio::mutable_buffer(probe_head + probe_len, kMaxProbeLen - probe_len),
                             [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
                                 if (!ec) {
                                     this->get_version_and_nmethods();
                                 } else {
                                     FDEBUG("Client {} Closed", convert::format_address(this->tcp_cli_endpoint));
                                     this->stop();
                                 }
                             });
        }
...
void Socks5Session::check_deadline() {
    ...
    if (deadline.expiry() <= asio::steady_timer::clock_type::now()) {
        FDEBUG("Client {} Timeout", convert::format_address(this->tcp_cli_endpoint));
        this->stop();
    } else {
        auto self = shared_from_this();
...
void Socks5Session::get_version_and_nmethods() {
    do {
        this->ver      = static_cast<decltype(this->ver)>(probe_head[0]);
        this->nmethods = static_cast<decltype(this->nmethods)>(probe_head[1]);
        FDEBUG("Client {} -> Proxy {} DATA : [VER = "
               "X'{:02x}', "
               "NMETHODS = {}]",
               convert::format_address(this->tcp_cli_endpoint), convert::format_address(this->local_endpoint),
               static_cast<int16_t>(this->ver), static_cast<int16_t>(this->nmethods));
        if (this->ver != SocksVersion::V5) {
            FDEBUG("Unsupported protocol version {}", static_cast<std::uint32_t>(this->ver));
            break;
        }
...
void Socks5Session::get_methods_list() {
    ...
    auto final_process_func = [this]() {
        FDEBUG("Client {} -> Proxy {} DATA : [METHODS = {}]", convert::format_address(this->tcp_cli_endpoint),
               convert::format_address(this->local_endpoint), this->methods_toString());
    ...
    if (this->methods.size() > 1) {
        auto self = shared_from_this();
        asio::async_read(this->socket, asio::buffer(this->methods.data() + 1, this->methods.size() - 1),
...
void Socks5Session::reply_support_method() {
    std::array<asio::const_buffer, 2> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->method, 1) } };

    auto self = shared_from_this();
    asio::async_write(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Proxy {} -> Client {} DATA : [VER = "
                   "X'{:02x}', "
                   "METHOD = X'{:02x}']",
                   convert::format_address(this->local_endpoint), convert::format_address(this->tcp_cli_endpoint),
                   static_cast<int16_t>(this->ver), static_cast<int16_t>(this->method));
...
void Socks5Session::get_username_length() {
    std::array<asio::mutable_buffer, 2> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->ulen, 1) } };

    auto self = shared_from_this();
    asio::async_read(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Client {} -> Proxy {} DATA : [VER = "
                   "X'{:02x}', ULEN = {}]",
                   convert::format_address(this->tcp_cli_endpoint), convert::format_address(this->local_endpoint),
                   static_cast<int16_t>(this->ver), static_cast<int16_t>(this->ulen));
...
void Socks5Session::get_username_content() {
    auto self = shared_from_this();
    asio::async_read(this->socket, asio::buffer(this->uname.data(), this->uname.size()),
                     [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
                         if (!ec) {
...
void Socks5Session::get_password_length() {
    ...
    asio::async_read(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Client {} -> Proxy {} DATA : [PLEN = {}]", convert::format_address(this->tcp_cli_endpoint),
                   convert::format_address(this->local_endpoint), static_cast<int16_t>(this->plen));
...
void Socks5Session::get_password_content() {
    auto self = shared_from_this();
    asio::async_read(this->socket, asio::buffer(this->passwd.data(), this->passwd.size()),
                     [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
                         if (!ec) {
...
void Socks5Session::do_auth_and_reply() {
    ...
    std::array<asio::const_buffer, 2> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->status, 1) } };
    ...
    asio::async_write(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Proxy {} -> Client {} DATA : [VER = "
                   "X'{:02x}', STATUS = X'{:02x}']",
                   convert::format_address(this->local_endpoint), convert::format_address(this->tcp_cli_endpoint),
                   static_cast<int16_t>(this->ver), static_cast<int16_t>(this->status));
...
void Socks5Session::get_request_from_client() {
    std::array<asio::mutable_buffer, 4> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->cmd, 1),
    ...
    asio::async_read(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Client {} -> Proxy {} DATA : [VER = X'{:02x}', CMD "
                   ...
                   convert::format_address(this->tcp_cli_endpoint), convert::format_address(this->local_endpoint),
                   static_cast<int16_t>(this->ver), static_cast<int16_t>(this->cmd), static_cast<int16_t>(this->rsv),
...
void Socks5Session::resolve_ipv4() {
    std::array<asio::mutable_buffer, 2> buf = { { asio::buffer(this->dst_addr.data(), this->dst_addr.size()),
...
void Socks5Session::resolve_domain() {
    this->resolve_domain_length();
}

void Socks5Session::resolve_domain_length() {
    std::array<asio::mutable_buffer, 1> buf = { asio::buffer(this->dst_addr.data(), 1) };
    ...
    asio::async_read(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            ...
            this->dst_addr.resize(static_cast<std::size_t>(this->dst_addr[0]));
...
void Socks5Session::reply_udp_associate() {
    ...
    try {
        if (this->udp_cli_endpoint.address().is_v4()) {
            ...
            std::memcpy(this->bnd_addr.data(), this->udp_socket->local_endpoint().address().to_v6().to_bytes().data(),
                        16);
        }
    ...
    std::array<asio::mutable_buffer, 6> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->rep, 1),
    ...
    asio::async_write(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Proxy {} -> Client {} DATA : [VER = "
                   ...
                   static_cast<int16_t>(this->reply_atyp), this->udp_bnd_endpoint.address().to_string(),
...
void Socks5Session::get_udp_client() {
    ...
    this->udp_socket->async_receive_from(
        asio::buffer(this->client_buffer.data(), this->client_buffer.size()), this->sender_endpoint,
        [this, self](asio::error_code ec, size_t length) {
            if (!ec) {
                this->udp_length = length;

                FDEBUG("UDP Client {} -> Proxy {} Data Length = {}", convert::format_address(this->sender_endpoint),
                       convert::format_address(this->udp_bnd_endpoint), this->udp_length);
...
bool Socks5Session::check_all_zeros() {
    return this->udp_cli_endpoint == asio::ip::udp::endpoint(this->udp_cli_endpoint.protocol(), 0);
}
...
void Socks5Session::parse_udp_message() {
    ...
    std::memcpy(&this->udp_rsv, this->client_buffer.data(), sizeof(this->udp_rsv));
    ...
    std::memcpy(&this->frag, this->client_buffer.data() + 2, sizeof(this->frag));
    if (this->frag != 0) {
    ...
    case SocksV5::ReplyATYP::Ipv4: {
        ...
        this->dst_addr.resize(4);
        std::memcpy(this->dst_addr.data(), this->client_buffer.data() + 4, 4);
        std::memcpy(&this->dst_port, this->client_buffer.data() + 8, sizeof(this->dst_port));
        this->dst_port = ntohs(this->dst_port);
        try {
        ...
        this->udp_length -= 10;
        std::memmove(this->client_buffer.data(), this->client_buffer.data() + 10, this->udp_length);
    ...
    case SocksV5::ReplyATYP::Ipv6: {
        ...
        std::memcpy(this->dst_addr.data(), this->client_buffer.data() + 4, 16);
        std::memcpy(&this->dst_port, this->client_buffer.data() + 20, sizeof(this->dst_port));
        this->dst_port = ntohs(this->dst_port);
        try {
            this->udp_dst_endpoint = asio::ip::udp::endpoint(
        ...
        this->udp_length -= 22;
        std::memmove(this->client_buffer.data(), this->client_buffer.data() + 22, this->udp_length);
    ...
    case SocksV5::ReplyATYP::DoMainName: {
        ...
        std::memcpy(this->dst_addr.data() + 1, this->client_buffer.data() + 5, domain_length);
        std::memcpy(&this->dst_port, this->client_buffer.data() + 5 + domain_length, sizeof(this->dst_port));
        this->dst_port = ntohs(this->dst_port);

        this->udp_length -= domain_length + 7;
        std::memmove(this->client_buffer.data(), this->client_buffer.data() + domain_length + 7, this->udp_length);
...
void Socks5Session::async_send_udp_message() {
    auto self = shared_from_this();
    this->udp_resolver.async_resolve(
        std::string(this->dst_addr.begin() + 1, this->dst_addr.end()), std::to_string(this->dst_port),
        [this, self](asio::error_code ec, const asio::ip::udp::resolver::results_type& result) {
            if (!ec) {
                this->resolve_results = result;

                FDEBUG("Reslove Domain {} {} result sets in total",
                       std::string(this->dst_addr.begin() + 1, this->dst_addr.end()), this->resolve_results.size());

                this->try_to_send_by_iterator(this->resolve_results.begin());
            } else {
                FWARN("Failed to Reslove Domain {}, ERR_MSG = [{}]",
                      std::string(this->dst_addr.begin() + 1, this->dst_addr.end()), ec.message());
...
void Socks5Session::try_to_send_by_iterator(asio::ip::udp::resolver::results_type::const_iterator iter) {
    ...
    FDEBUG("Try to Send {}", convert::format_address(this->udp_dst_endpoint));

    ++iter;

    auto self = shared_from_this();
    this->udp_socket->async_send_to(asio::buffer(this->client_buffer.data(), this->udp_length), this->udp_dst_endpoint,
                                    [this, self, iter](asio::error_code ec, size_t length) {
                                        if (!ec) {
                                            FTRACE("Proxy {} -> UDP Server {} Data Length = {}",
                                                   ...
                                                   convert::format_address(this->udp_dst_endpoint), length);
...
void Socks5Session::send_udp_to_dst() {
    ...
    udp_socket->async_send_to(
        asio::buffer(this->client_buffer.data(), this->udp_length), this->udp_dst_endpoint,
        [this, self](asio::error_code ec, size_t length) {
            if (!ec) {
                FTRACE("Proxy {} -> UDP Server {} Data Length = {}", convert::format_address(this->udp_bnd_endpoint),
                       convert::format_address(this->udp_dst_endpoint), length);

                this->keep_alive();
                this->receive_udp_message();
            } else {
                FWARN("Failed to send message to UDP Server {}", convert::format_address(this->udp_dst_endpoint));
                this->stop();
            }
        });
}
...
void Socks5Session::send_udp_to_client() {
    ...
    std::array<asio::const_buffer, 5> buf = {
        { asio::buffer(&this->udp_rsv, 2), asio::buffer(&this->frag, 1), asio::buffer(&this->reply_atyp, 1),
          asio::buffer(this->dst_addr.data(), this->dst_addr.size()), asio::buffer(&this->dst_port, 2) }
    };
    ...
    std::memmove(this->client_buffer.data() + buf_bytes, this->client_buffer.data(), this->udp_length);
    ...
    this->udp_socket->async_send_to(
        asio::buffer(this->client_buffer.data(), this->udp_length), this->udp_cli_endpoint,
        [this, self](asio::error_code ec, size_t length) {
            if (!ec) {
                FTRACE("Proxy {} -> UDP Client {} Data Length = {}", convert::format_address(this->udp_bnd_endpoint),
                       convert::format_address(this->udp_cli_endpoint), length);

                this->keep_alive();
                this->receive_udp_message();
            } else {
                FWARN("Failed to send message to UDP Client {}", convert::format_address(this->udp_cli_endpoint));
...
void Socks5Session::receive_udp_message() {
    auto self = shared_from_this();
    this->udp_socket->async_receive_from(asio::buffer(this->client_buffer.data(), this->client_buffer.size()),
                                         this->sender_endpoint, [this, self](asio::error_code ec, size_t length) {
                                             if (!ec) {
                                                 this->udp_length = length;
                                                 ...
                                                 if (this->sender_endpoint == this->udp_cli_endpoint) {
                                                     FTRACE("UDP Client {} -> Proxy {} Data Length = {}",
                                                            ...
                                                            convert::format_address(this->udp_bnd_endpoint), length);
...
void Socks5Session::connect_dst_host() {
    auto self = shared_from_this();
    this->dst_socket.async_connect(this->tcp_dst_endpoint, [this, self](asio::error_code ec) {
        if (!ec) {
            try {
            ...
            FDEBUG("Proxy {} -> Server {} Connection Successed", convert::format_address(this->tcp_bnd_endpoint),
                   convert::format_address(this->tcp_dst_endpoint));

            this->reply_connect_result();
        } else {
            FDEBUG("Server {} Connection Failed", convert::format_address(this->tcp_dst_endpoint));
            this->stop();
        }
    });
}
...
void Socks5Session::try_to_connect_by_iterator(asio::ip::udp::resolver::results_type::const_iterator iter) {
    ...
    this->tcp_dst_endpoint = asio::ip::tcp::endpoint(iter->endpoint().address(), iter->endpoint().port());

    FDEBUG("Try to Connect {}", convert::format_address(this->tcp_dst_endpoint));

    ++iter;

    auto self = shared_from_this();
    this->dst_socket.async_connect(this->tcp_dst_endpoint, [this, self, iter](asio::error_code ec) {
        if (!ec) {
            try {
                this->tcp_bnd_endpoint = this->dst_socket.local_endpoint();
            } catch (const asio::system_error&) {
...
void Socks5Session::reply_and_stop(SocksV5::ReplyREP rep) {
    ...
    std::array<asio::const_buffer, 6> buf = { { asio::buffer(&this->ver, 1), asio::buffer(&this->rep, 1),
                                                asio::buffer(&this->rsv, 1), asio::buffer(&this->reply_atyp, 1),
                                                asio::buffer(this->bnd_addr.data(), this->bnd_addr.size()),
                                                asio::buffer(&this->bnd_port, 2) } };
    ...
    asio::async_write(this->socket, buf, [this, self](asio::error_code ec, size_t /*bytes_transferred*/) {
        if (!ec) {
            FDEBUG("Proxy {} -> Client {} DATA : [VER = X'{:02x}', REP "
                   ...
                   convert::format_address(this->local_endpoint), convert::format_address(this->tcp_cli_endpoint),
                   static_cast<int16_t>(this->ver), static_cast<int16_t>(this->rep), static_cast<int16_t>(this->rsv),
                   static_cast<int16_t>(this->reply_atyp), this->tcp_bnd_endpoint.address().to_string(),
...
void Socks5Session::read_from_client() {
    auto self = shared_from_this();
    this->socket.async_read_some(asio::buffer(this->client_buffer.data(), this->client_buffer.size()),
                                 [this, self](asio::error_code ec, size_t length) {
                                     if (!ec) {
                                         FTRACE("Client {} -> Proxy {} Data Length = {}",
                                                convert::format_address(this->tcp_cli_endpoint),
                                                convert::format_address(this->local_endpoint), length);

                                         this->keep_alive();
                                         this->send_to_dst(length);
                                     } else {
                                         FTRACE("Client {} Closed", convert::format_address(this->tcp_cli_endpoint));
                                         this->stop();
                                     }
                                 });
}
...
void Socks5Session::send_to_dst(size_t write_length) {
    auto self = shared_from_this();
    asio::async_write(this->dst_socket, asio::buffer(this->client_buffer.data(), write_length),
                      [this, self](asio::error_code ec, size_t length) {
                          if (!ec) {
                              FTRACE("Proxy {} -> Server {} Data Length = {}",
                                     convert::format_address(this->tcp_bnd_endpoint),
                                     convert::format_address(this->tcp_dst_endpoint), length);

                              this->keep_alive();
                              this->read_from_client();
                          } else {
                              FTRACE("Server {} Closed", convert::format_address(this->tcp_dst_endpoint));
                              this->stop();
                          }
                      });
}
...
void Socks5Session::read_from_dst() {
    ...
    this->dst_socket.async_read_some(
        ...
        [this, self](asio::error_code ec, size_t length) {
            if (!ec) {
                FTRACE("Server {} -> Proxy {} Data Length = {}", convert::format_address(this->tcp_dst_endpoint),
                       convert::format_address(this->tcp_bnd_endpoint), length);
...
void Socks5Session::send_to_client(size_t write_length) {
    auto self = shared_from_this();
    asio::async_write(this->socket, asio::buffer(this->dst_buffer.data(), write_length),
                      [this, self](asio::error_code ec, size_t length) {
                          if (!ec) {
                              FTRACE("Proxy {} -> Client {} Data Length = {}",
                                     convert::format_address(this->local_endpoint),
                                     convert::format_address(this->tcp_cli_endpoint), length);
