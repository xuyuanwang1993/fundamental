    ...
    void generate_report() const {
        ...
        for (const auto& bench_group : m_all_benchmarks) {
            ...
            for (const auto& bench_list : bench_group.second) {
                ...
                const auto std_dev_magnitude =
                    detail::get_magnitude(bench_list.second.m_analysis.standard_deviation.point);
                benchmark_data["std_dev"] =
                    bench_list.second.m_analysis.standard_deviation.point.count() * std_dev_magnitude;
                benchmark_data["std_dev_unit"] = detail::units_for_magnitude(std_dev_magnitude);
                const auto std_dev_lb_magnitude =
                    detail::get_magnitude(bench_list.second.m_analysis.standard_deviation.lower_bound);
                benchmark_data["std_dev_lb"] =
                    bench_list.second.m_analysis.standard_deviation.lower_bound.count() * std_dev_lb_magnitude;
                benchmark_data["std_dev_lb_unit"] = detail::units_for_magnitude(std_dev_lb_magnitude);
                const auto std_dev_ub_magnitude =
                    detail::get_magnitude(bench_list.second.m_analysis.standard_deviation.upper_bound);
                benchmark_data["std_dev_ub"] =
                    bench_list.second.m_analysis.standard_deviation.upper_bound.count() * std_dev_ub_magnitude;
                benchmark_data["std_dev_ub_unit"] = detail::units_for_magnitude(std_dev_ub_magnitude);
                benchmark_data["std_dev_ci"] =
                    truncate(bench_list.second.m_analysis.standard_deviation.confidence_interval);
...
private:
    ...
    static std::string get_outliers_analysis(sample_analysis<fp_seconds> const& analysis) {
        ...
        return std::string("Found ") + std::to_string(o.total()) + std::string(" outliers among ") +
               std::to_string(o.samples_seen) + std::string(" samples (") +
               detail::percentage_ratio(o.total(), o.samples_seen) + std::string(")");
    }
    ...
    static std::string get_outliers_effect(sample_analysis<fp_seconds> const& analysis) {
        ...
        // std::string tmp = std::string("variance introduced by outliers: ") +
        // detail::percentage(analysis.outlier_variance) + "\n";
...
#include <nonius/html_group_reporter_template.h>
    ...
    void do_measurement_start(execution_plan<fp_seconds> plan) override {
        ...
        if (!summary)
            std::cout << "collecting " << n_samples << " samples, " << plan.iterations_per_sample
                      << " iterations each, in estimated " << detail::pretty_duration(plan.estimated_duration) << "\n";
    }
    ...
    void do_analysis_complete(sample_analysis<fp_seconds> const& analysis) override {
        ...
        if (verbose)
            std::cout << "variance introduced by outliers: " << detail::percentage(analysis.outlier_variance) << "\n";
        const char* effect;
        if (analysis.outlier_variance < 0.01) {
            effect = "unaffected";
        } else if (analysis.outlier_variance < 0.1) {
    ...
    void print_outlier_count(const char* description, int count, int total) {
        if (count > 0)
            std::cout << "  " << count << " (" << detail::percentage_ratio(count, total) << ") " << description << "\n";
    }
    void print_outliers(outlier_classification o) {
        std::cout << "found " << o.total() << " outliers among " << o.samples_seen << " samples ("
                  << detail::percentage_ratio(o.total(), o.samples_seen) << ")\n";
        if (verbose) {
    ...
    void print_statistic_estimate(const char* name, estimate<fp_seconds> estimate) {
        ...
        if (!summary) {
            std::cout << ", lb " << detail::pretty_duration(estimate.lower_bound) << ", ub "
                      << detail::pretty_duration(estimate.upper_bound) << ", ci " << std::setprecision(3)
        ...
        for (auto bench_groups : m_all_benchmarks) {
            for (auto bench_list : bench_groups.second) {
                if (!bench_list.second.m_sample_data.empty())
                    mins.push_back(*std::min_element(bench_list.second.m_sample_data.begin(),
                                                     bench_list.second.m_sample_data.end()));
            }
        }
