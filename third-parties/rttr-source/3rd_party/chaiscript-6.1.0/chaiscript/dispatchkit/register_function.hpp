...
Proxy_Function fun(const T& t) {
    ...
    return Proxy_Function(
        chaiscript::make_shared<dispatch::Proxy_Function_Base, dispatch::Proxy_Function_Callable_Impl<Signature, T>>(
            t));
}
...
Proxy_Function fun(Ret (*func)(Param...)) {
    ...
    return Proxy_Function(
        chaiscript::make_shared<dispatch::Proxy_Function_Base,
                                dispatch::Proxy_Function_Callable_Impl<Ret(Param...), decltype(fun_call)>>(fun_call));
}
...
Proxy_Function fun(Ret (Class::*t_func)(Param...) const) {
    ...
    return Proxy_Function(
        chaiscript::make_shared<dispatch::Proxy_Function_Base,
                                dispatch::Proxy_Function_Callable_Impl<Ret(const Class&, Param...), decltype(call)>>(
            call));
}
...
Proxy_Function fun(Ret (Class::*t_func)(Param...)) {
    ...
    return Proxy_Function(
        chaiscript::make_shared<dispatch::Proxy_Function_Base,
                                dispatch::Proxy_Function_Callable_Impl<Ret(Class&, Param...), decltype(call)>>(call));
}

template <typename T,
          typename Class /*, typename = typename std::enable_if<std::is_member_object_pointer<T>::value>::type*/>
Proxy_Function fun(T Class::*m /*, typename std::enable_if<std::is_member_object_pointer<T>::value>::type* = 0*/) {
    return Proxy_Function(
        chaiscript::make_shared<dispatch::Proxy_Function_Base, dispatch::Attribute_Access<T, Class>>(m));
}

// only compile this bit if noexcept is part of the type system
//
#if (defined(__cpp_noexcept_function_type) && __cpp_noexcept_function_type >= 201510) ||                               \
    (defined(_NOEXCEPT_TYPES_SUPPORTED) && _MSC_VER >= 1912)
template <typename Ret, typename... Param>
...
/// \brief Creates a new Proxy_Function object from a free function, member function or data member and binds the first
/// parameter of it \param[in] t Function / member to expose \param[in] q Value to bind to first parameter
///
/// \b Example:
/// \code
/// struct MyClass
