...
private:
    struct Data {
        explicit Data(const std::type_info& t_type) : m_type(t_type) {
        }
...
public:
    ...
    m_data(
        std::unique_ptr<Data>(new Data_Impl<typename std::decay<ValueType>::type>(std::forward<ValueType>(t_value)))) {
    }
