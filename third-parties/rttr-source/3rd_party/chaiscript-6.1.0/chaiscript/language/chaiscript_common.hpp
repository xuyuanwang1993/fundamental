...
struct Name_Validator {
    static bool is_reserved_word(const std::string& name) {
        static const std::set<std::string> m_reserved_words = { "def",      "fun",      "while",    "for",      "if",
                                                                "else",     "&&",       "||",       ",",        "auto",
                                                                "return",   "break",    "true",     "false",    "class",
                                                                "attr",     "var",      "global",   "GLOBAL",   "_",
...
enum class Operator_Precidence {
    Ternary_Cond,
    Logical_Or,
    Logical_And,
    Bitwise_Or,
...
inline const char* ast_node_type_to_string(AST_Node_Type ast_node_type) {
    static const char* const ast_node_types[] = { "Id",          "Fun_Call",    "Unused_Return_Fun_Call",
...
struct File_Position {
    ...
    File_Position(int t_file_line, int t_file_column) : line(t_file_line), column(t_file_column) {
    }
...
struct Parse_Location {
    ...
    Parse_Location(std::shared_ptr<std::string> t_fname, const int t_start_line = 0, const int t_start_col = 0,
                   const int t_end_line = 0, const int t_end_col = 0) :
    start(t_start_line, t_start_col),
    end(t_end_line, t_end_col), filename(std::move(t_fname)) {
    }
...
struct load_module_error : std::runtime_error {
    explicit load_module_error(const std::string& t_reason) noexcept : std::runtime_error(t_reason) {
    }
...
struct eval_error : std::runtime_error {
    ...
    eval_error(const std::string& t_why, const std::vector<Boxed_Value>& t_parameters,
               const std::vector<chaiscript::Const_Proxy_Function>& t_functions, bool t_dot_notation,
               const chaiscript::detail::Dispatch_Engine& t_ss) noexcept :
    ...
    std::runtime_error(format(t_why, t_where, t_fname)), reason(t_why), start_position(t_where), filename(t_fname) {
    }
        ...
        if (!call_stack.empty()) {
            ...
            for (size_t j = 1; j < call_stack.size(); ++j) {
                if (id(call_stack[j]) != chaiscript::AST_Node_Type::Block &&
                    ...
                    ss << '\n';
                    ss << "  from " << fname(call_stack[j]) << " (" << startpos(call_stack[j]) << ") '"
                       << pretty(call_stack[j]) << "'";
                }
            }
        }
...
private:
    ...
    static std::string format_types(const Const_Proxy_Function& t_func, bool t_dot_notation,
    ...
    static std::string format_detail(const std::vector<chaiscript::Const_Proxy_Function>& t_functions,
                                     bool t_dot_notation, const chaiscript::detail::Dispatch_Engine& t_ss) {
        std::stringstream ss;
    ...
    static std::string format(const std::string& t_why, const std::vector<Boxed_Value>& t_parameters,
                              bool t_dot_notation, const chaiscript::detail::Dispatch_Engine& t_ss) {
        std::stringstream ss;
...
struct file_not_found_error : std::runtime_error {
    ...
    std::runtime_error("File Not Found: " + t_filename), filename(t_filename) {
    }
...
public:
    ...
    std::string to_string(const std::string& t_prepend = "") const {
        ...
        oss << t_prepend << "(" << ast_node_type_to_string(this->identifier) << ") " << this->text << " : "
...
struct AST_Node_Trace {
    ...
    identifier(node.identifier), text(node.text), location(node.location), children(get_children(node)) {
    }
...
struct Scope_Push_Pop {
    ...
    explicit Scope_Push_Pop(const chaiscript::detail::Dispatch_State& t_ds) : m_ds(t_ds) {
...
struct Function_Push_Pop {
    ...
    explicit Function_Push_Pop(const chaiscript::detail::Dispatch_State& t_ds) : m_ds(t_ds) {
...
struct Stack_Push_Pop {
    ...
    explicit Stack_Push_Pop(const chaiscript::detail::Dispatch_State& t_ds) : m_ds(t_ds) {
