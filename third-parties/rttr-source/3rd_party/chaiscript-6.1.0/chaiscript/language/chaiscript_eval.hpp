...
template <typename T>
static Boxed_Value eval_function(chaiscript::detail::Dispatch_Engine& t_ss, const AST_Node_Impl<T>& t_node,
...
inline Boxed_Value clone_if_necessary(Boxed_Value incoming, std::atomic_uint_fast32_t& t_loc,
...
struct AST_Node_Impl : AST_Node {
    ...
    static bool get_scoped_bool_condition(const AST_Node_Impl<T>& node,
                                          const chaiscript::detail::Dispatch_State& t_ss) {
...
struct Compiled_AST_Node : AST_Node_Impl<T> {
    Compiled_AST_Node(AST_Node_Impl_Ptr<T> t_original_node, std::vector<AST_Node_Impl_Ptr<T>> t_children,
                      std::function<Boxed_Value(const std::vector<AST_Node_Impl_Ptr<T>>&,
                                                const chaiscript::detail::Dispatch_State& t_ss)>
                          t_func) :
    AST_Node_Impl<T>(t_original_node->text, AST_Node_Type::Compiled, t_original_node->location, std::move(t_children)),
    m_func(std::move(t_func)), m_original_node(std::move(t_original_node)) {
    }
    ...
    std::function<Boxed_Value(const std::vector<AST_Node_Impl_Ptr<T>>&, const chaiscript::detail::Dispatch_State& t_ss)>
        m_func;
...
struct Fold_Right_Binary_Operator_AST_Node : AST_Node_Impl<T> {
    Fold_Right_Binary_Operator_AST_Node(const std::string& t_oper, Parse_Location t_loc,
                                        std::vector<AST_Node_Impl_Ptr<T>> t_children, Boxed_Value t_rhs) :
    AST_Node_Impl<T>(t_oper, AST_Node_Type::Binary, std::move(t_loc), std::move(t_children)),
    m_oper(Operators::to_operator(t_oper)), m_rhs(std::move(t_rhs)) {
    }
...
protected:
    Boxed_Value do_oper(const chaiscript::detail::Dispatch_State& t_ss, const std::string& t_oper_string,
                        const Boxed_Value& t_lhs) const {
        try {
            ...
            throw exception::eval_error("Can not find appropriate '" + t_oper_string + "' operator.", e.parameters,
                                        e.functions, false, *t_ss);
        }
    }
...
struct Binary_Operator_AST_Node : AST_Node_Impl<T> {
    Binary_Operator_AST_Node(const std::string& t_oper, Parse_Location t_loc,
...
protected:
    Boxed_Value do_oper(const chaiscript::detail::Dispatch_State& t_ss, Operators::Opers t_oper,
                        const std::string& t_oper_string, const Boxed_Value& t_lhs, const Boxed_Value& t_rhs) const {
        try {
            if (t_oper != Operators::Opers::invalid && t_lhs.get_type_info().is_arithmetic() &&
                t_rhs.get_type_info().is_arithmetic()) {
                // If it's an arithmetic operation we want to short circuit dispatch
                try {
...
struct Constant_AST_Node final : AST_Node_Impl<T> {
    ...
    AST_Node_Impl<T>(t_ast_node_text, AST_Node_Type::Constant, std::move(t_loc)), m_value(std::move(t_value)) {
    }

    explicit Constant_AST_Node(Boxed_Value t_value) :
    AST_Node_Impl<T>("", AST_Node_Type::Constant, Parse_Location()), m_value(std::move(t_value)) {
    }
    ...
    Boxed_Value do_eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const {
        ...
        try {
            ...
            throw exception::eval_error(std::string(e.what()) + " with function '" + this->children[0]->text + "'",
                                        e.parameters, e.functions, false, *t_ss);
            ...
            try {
                ...
                throw exception::eval_error("Error calling function '" + this->children[0]->text + "'", params, { f },
                                            false, *t_ss);
            } catch (const exception::bad_boxed_cast&) {
                throw exception::eval_error("'" + this->children[0]->pretty_print() +
...
struct Unused_Return_Fun_Call_AST_Node final : Fun_Call_AST_Node<T> {
    Unused_Return_Fun_Call_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                                    std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Arg_List_AST_Node final : AST_Node_Impl<T> {
    ...
    static std::pair<std::string, Type_Info> get_arg_type(const AST_Node_Impl<T>& t_node,
                                                          const chaiscript::detail::Dispatch_State& t_ss) {
    ...
    static dispatch::Param_Types get_arg_types(const AST_Node_Impl<T>& t_node,
                                               const chaiscript::detail::Dispatch_State& t_ss) {
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        if (m_oper != Operators::Opers::invalid && lhs.get_type_info().is_arithmetic() &&
            ...
            try {
                ...
                throw exception::eval_error("Unable to find appropriate'" + this->text + "' operator.", e.parameters,
                                            e.functions, false, *t_ss);
            }
        }
...
struct Global_Decl_AST_Node final : AST_Node_Impl<T> {
    Global_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Assign_Decl_AST_Node final : AST_Node_Impl<T> {
    Assign_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Array_Call_AST_Node final : AST_Node_Impl<T> {
    Array_Call_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        try {
            ...
            throw exception::eval_error("Can not find appropriate array lookup operator '[]'.", e.parameters,
                                        e.functions, false, *t_ss);
        }
    }
...
struct Dot_Access_AST_Node final : AST_Node_Impl<T> {
    Dot_Access_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Dot_Access, std::move(t_loc), std::move(t_children)),
    m_fun_name(((this->children[1]->identifier == AST_Node_Type::Fun_Call) ||
                (this->children[1]->identifier == AST_Node_Type::Array_Call))
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        try {
            ...
            if (e.functions.empty()) {
                ...
                throw exception::eval_error(std::string(e.what()) + " for function '" + m_fun_name + "'", e.parameters,
                                            e.functions, true, *t_ss);
            }
        } catch (detail::Return_Value& rv) {
        ...
        if (this->children[1]->identifier == AST_Node_Type::Array_Call) {
            try {
                retval = t_ss->call_function("[]", m_array_loc, { retval, this->children[1]->children[1]->eval(t_ss) },
                                             t_ss.conversions());
            } catch (const exception::dispatch_error& e) {
                throw exception::eval_error("Can not find appropriate array lookup operator '[]'.", e.parameters,
                                            e.functions, true, *t_ss);
...
struct Lambda_AST_Node final : AST_Node_Impl<T> {
    ...
    AST_Node_Impl<T>(t_ast_node_text, AST_Node_Type::Lambda, std::move(t_loc),
    ...
    m_this_capture(has_this_capture(this->children[0]->children)), m_lambda_node(std::move(t_children.back())) {
    }
    ...
    static bool has_this_capture(const std::vector<AST_Node_Impl_Ptr<T>>& children) {
        return std::any_of(std::begin(children), std::end(children),
                           [](const auto& child) { return child->children[0]->text == "this"; });
    }
...
struct Scopeless_Block_AST_Node final : AST_Node_Impl<T> {
    Scopeless_Block_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                             std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Scopeless_Block, std::move(t_loc),
                     std::move(t_children)) {
    }
...
struct Def_AST_Node final : AST_Node_Impl<T> {
    ...
    AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Def, std::move(t_loc),
                     std::vector<AST_Node_Impl_Ptr<T>>(
                         std::make_move_iterator(t_children.begin()),
...
struct Ranged_For_AST_Node final : AST_Node_Impl<T> {
    Ranged_For_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        const auto get_function = [&t_ss](const std::string& t_name, auto& t_hint) {
            ...
            if (funs.first != hint) {
                t_hint = uint_fast32_t(funs.first);
            }
...
struct For_AST_Node final : AST_Node_Impl<T> {
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        try {
            for (this->children[0]->eval(t_ss); this->get_scoped_bool_condition(*this->children[1], t_ss);
                 this->children[2]->eval(t_ss)) {
                try {
                    // Body of Loop
        ...
        while (!breaking && (currentCase < this->children.size())) {
            try {
                if (this->children[currentCase]->identifier == AST_Node_Type::Case) {
                    // This is a little odd, but because want to see both the switch and the case simultaneously, I do a
                    // downcast here.
                    try {
                        if (hasMatched ||
                            boxed_cast<bool>(t_ss->call_function(
                                "==", m_loc, { match_value, this->children[currentCase]->children[0]->eval(t_ss) },
                                t_ss.conversions()))) {
...
struct Inline_Array_AST_Node final : AST_Node_Impl<T> {
    Inline_Array_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                          std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Inline_Map_AST_Node final : AST_Node_Impl<T> {
    Inline_Map_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        try {
            ...
            throw exception::eval_error(
                "Can not find appropriate copy constructor or 'clone' while inserting into Map.", e.parameters,
                e.functions, false, *t_ss);
        }
    }
...
struct Reference_AST_Node final : AST_Node_Impl<T> {
    Reference_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                       std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Prefix_AST_Node final : AST_Node_Impl<T> {
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        try {
            ...
            if (m_oper != Operators::Opers::invalid && m_oper != Operators::Opers::bitwise_and &&
                bv.get_type_info().is_arithmetic()) {
                if ((m_oper == Operators::Opers::pre_increment || m_oper == Operators::Opers::pre_decrement) &&
                    bv.is_const()) {
            ...
            throw exception::eval_error("Error with prefix operator evaluation: '" + this->text + "'", e.parameters,
                                        e.functions, false, *t_ss);
        }
    }
...
struct Noop_AST_Node final : AST_Node_Impl<T> {
    Noop_AST_Node() : AST_Node_Impl<T>("", AST_Node_Type::Noop, Parse_Location()) {
    }
...
struct Value_Range_AST_Node final : AST_Node_Impl<T> {
    Value_Range_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Inline_Range_AST_Node final : AST_Node_Impl<T> {
    Inline_Range_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                          std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        try {
            ...
            throw exception::eval_error("Unable to generate range vector, while calling 'generate_range'", e.parameters,
                                        e.functions, false, *t_ss);
        }
    }
    ...
    Boxed_Value handle_exception(const chaiscript::detail::Dispatch_State& t_ss, const Boxed_Value& t_except) const {
        ...
        for (size_t i = 1; i < end_point; ++i) {
            ...
            if (catch_block.children.size() == 1) {
                ...
                if (dispatch::Param_Types(std::vector<std::pair<std::string, Type_Info>> {
                                              Arg_List_AST_Node<T>::get_arg_type(*catch_block.children[0], t_ss) })
...
struct Method_AST_Node final : AST_Node_Impl<T> {
    ...
    AST_Node_Impl<T>(
        std::move(t_ast_node_text), AST_Node_Type::Method, std::move(t_loc),
        std::vector<AST_Node_Impl_Ptr<T>>(
            std::make_move_iterator(t_children.begin()),
    ...
    m_guard_node(
        Def_AST_Node<T>::get_guard_node(std::move(t_children), t_children.size() - this->children.size() == 2)) {
    }
...
struct Attr_Decl_AST_Node final : AST_Node_Impl<T> {
    Attr_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                       std::vector<AST_Node_Impl_Ptr<T>> t_children) :
    ...
    Boxed_Value eval_internal(const chaiscript::detail::Dispatch_State& t_ss) const override {
        ...
        try {
            ...
            t_ss->add(std::make_shared<dispatch::detail::Dynamic_Object_Function>(
                          std::move(class_name),
                          fun([attr_name](dispatch::Dynamic_Object& t_obj) { return t_obj.get_attr(attr_name); }), true
...
struct Logical_And_AST_Node final : AST_Node_Impl<T> {
    Logical_And_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children) :
...
struct Logical_Or_AST_Node final : AST_Node_Impl<T> {
    Logical_Or_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children) :
