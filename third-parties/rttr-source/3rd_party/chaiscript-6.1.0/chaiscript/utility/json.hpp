...
private:
    ...
    struct Internal {
        ...
        List(clone(other.List)), Map(clone(other.Map)), String(clone(other.String)), Float(other.Float), Int(other.Int),
        Bool(other.Bool), Type(other.Type) {
        }
...
public:
    ...
    explicit JSON(Class type) : internal(type) {
    }

    JSON(initializer_list<JSON> list) : internal(Class::Object) {
    ...
    template <typename T>
    explicit JSON(T i, typename enable_if<is_integral<T>::value && !is_same<T, bool>::value>::type* = nullptr) :
    internal(static_cast<int64_t>(i)) {
    }

    template <typename T>
    explicit JSON(T f, typename enable_if<is_floating_point<T>::value>::type* = nullptr) :
    ...
    bool is_null() const {
        return internal.Type == Class::Null;
    }
...
struct JSONParser {
    ...
    static JSON parse_array(const std::string& str, size_t& offset) {
        ...
        for (; offset < str.size();) {
            ...
            if (str.at(offset) == ',') {
                ...
                throw std::runtime_error(std::string("JSON ERROR: Array: Expected ',' or ']', found '") +
                                         str.at(offset) + "'\n");
            }
        }
                    ...
                    for (size_t i = 1; i <= 4; ++i) {
                        ...
                        if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')) {
                            ...
                            throw std::runtime_error(
                                std::string("JSON ERROR: String: Expected hex character in unicode escape, found '") +
                                c + "'");
                        }
                    }
                    offset += 4;
    ...
    static JSON parse_number(const std::string& str, size_t& offset) {
        ...
        if (offset < str.size() && (c == 'E' || c == 'e')) {
            ...
            for (; offset < str.size();) {
                ...
                if (c >= '0' && c <= '9') {
                    ...
                    throw std::runtime_error(
                        std::string("JSON ERROR: Number: Expected a number for exponent, found '") + c + "'");
                } else {
                    break;
                }
            }
        ...
        if (isDouble) {
            ...
            if (!exp_str.empty()) {
                return JSON((isNegative ? -1 : 1) * static_cast<double>(chaiscript::parse_num<int64_t>(val)) *
                            std::pow(10, exp));
            } else {
    ...
    static JSON parse_bool(const std::string& str, size_t& offset) {
        if (str.substr(offset, 4) == "true") {
            ...
            throw std::runtime_error(std::string("JSON ERROR: Bool: Expected 'true' or 'false', found '") +
                                     str.substr(offset, 5) + "'");
        }
    }
