...
void array(const std::string& type, Module& m) {
    ...
    m.add(fun([extent](T& t, size_t index) -> ReturnType& {
              if (extent > 0 && index >= extent) {
                  throw std::range_error("Array index out of range. Received: " + std::to_string(index) +
                                         " expected < " + std::to_string(extent));
              } else {
                  return t[index];
              }
          }),
    ...
    m.add(fun([extent](const T& t, size_t index) -> const ReturnType& {
              if (extent > 0 && index >= extent) {
                  throw std::range_error("Array index out of range. Received: " + std::to_string(index) +
                                         " expected < " + std::to_string(extent));
              } else {
    ...
    m.add(fun([extent](const T&) { return extent; }), "size");
}
...
auto parse_string(const std::string& i) ->
    typename std::enable_if<!std::is_same<Input, wchar_t>::value && !std::is_same<Input, char16_t>::value &&
...
template <typename Type>
std::shared_ptr<typename std::remove_const<Type>::type> shared_ptr_unconst_clone(
    const std::shared_ptr<typename std::add_const<Type>::type>& p) {
...
Boxed_Value ptr_assign(Boxed_Value lhs, const std::shared_ptr<Type>& rhs) {
    if (lhs.is_undef() || (!lhs.get_type_info().is_const() &&
                           lhs.get_type_info().bare_equal(chaiscript::detail::Get_Type_Info<Type>::get()))) {
...
private:
    ...
    static Boxed_Value bind_function(const std::vector<Boxed_Value>& params) {
        ...
        return Boxed_Value(Const_Proxy_Function(std::make_shared<dispatch::Bound_Function>(
            std::move(f), std::vector<Boxed_Value>(params.begin() + 1, params.end()))));
    }
...
public:
    ...
    static void bootstrap(Module& m) {
        ...
        m.add(fun(return_boxed_value_vector(&dispatch::Proxy_Function_Base::get_contained_functions)),
        ...
        m.add(fun(static_cast<Boxed_Value& (dispatch::Dynamic_Object::*)(const std::string&)>(
                  &dispatch::Dynamic_Object::get_attr)),
              "get_attr");
        ...
        m.add(fun([](dispatch::Assignable_Proxy_Function& t_lhs,
                     const std::shared_ptr<const dispatch::Proxy_Function_Base>& t_rhs) { t_lhs.assign(t_rhs); }),
              "=");
        ...
        chaiscript::utility::add_class<chaiscript::exception::eval_error>(
            m, "eval_error", {},
            { { fun(&chaiscript::exception::eval_error::reason), "reason" },
              ...
              { fun([](const chaiscript::exception::eval_error& t_eval_error) {
                    ...
                    std::transform(t_eval_error.call_stack.begin(), t_eval_error.call_stack.end(),
                                   std::back_inserter(retval), &chaiscript::var<const chaiscript::AST_Node_Trace&>);
                    return retval;
                }),
                "call_stack" } });

        chaiscript::utility::add_class<chaiscript::File_Position>(
            m, "File_Position", { constructor<File_Position()>(), constructor<File_Position(int, int)>() },
            { { fun(&File_Position::line), "line" }, { fun(&File_Position::column), "column" } });

        chaiscript::utility::add_class<AST_Node>(
            m, "AST_Node", {},
            { { fun(&AST_Node::text), "text" },
              ...
              { fun([](const chaiscript::AST_Node& t_node) -> std::vector<Boxed_Value> {
                    ...
                    std::transform(children.begin(), children.end(), std::back_inserter(retval),
