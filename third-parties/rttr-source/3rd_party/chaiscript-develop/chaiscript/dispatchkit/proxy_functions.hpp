...
std::function<FunctionType> functor(std::shared_ptr<const Proxy_Function_Base> func,
                                    const Type_Conversions_State* t_conversions);

class Param_Types {
public:
    Param_Types() : m_has_types(false), m_doti(user_type<Dynamic_Object>()) {
    }

    explicit Param_Types(std::vector<std::pair<std::string, Type_Info>> t_types) :
    m_types(std::move(t_types)), m_has_types(false), m_doti(user_type<Dynamic_Object>()) {
        update_has_types();
    }
                        ...
                        if (!bv.get_type_info().bare_equal(ti)) {
                            if (t_conversions->converts(ti, bv.get_type_info())) {
                                try {
                                    ...
                                    vals[i] = t_conversions->boxed_type_conversion(m_types[i].second,
                                                                                   t_conversions.saves(), vals[i]);
                                } catch (...) {
                                    try {
                                        // try going the other way
                                        vals[i] = t_conversions->boxed_type_down_conversion(
                                            m_types[i].second, t_conversions.saves(), vals[i]);
    ...
    std::pair<bool, bool> match(const std::vector<Boxed_Value>& vals,
...
public:
    ...
    Boxed_Value operator()(const std::vector<Boxed_Value>& params,
                           const chaiscript::Type_Conversions_State& t_conversions) const {
    ...
    const std::vector<Type_Info>& get_param_types() const {
        return m_types;
    }

    virtual bool operator==(const Proxy_Function_Base&) const                  = 0;
    virtual bool call_match(const std::vector<Boxed_Value>& vals,
                            const Type_Conversions_State& t_conversions) const = 0;

    virtual bool is_attribute_function() const {
        return false;
    }
    ...
    static bool compare_type_to_param(const Type_Info& ti, const Boxed_Value& bv,
                                      const Type_Conversions_State& t_conversions) {
        if (ti.is_undef() || ti.bare_equal(user_type<Boxed_Value>()) ||
...
protected:
    virtual Boxed_Value do_call(const std::vector<Boxed_Value>& params,
                                const Type_Conversions_State& t_conversions) const = 0;
...
public:
    Dynamic_Proxy_Function(const int t_arity, std::shared_ptr<AST_Node> t_parsenode,
                           Param_Types t_param_types = Param_Types(), Proxy_Function t_guard = Proxy_Function()) :
    Proxy_Function_Base(build_param_type_list(t_param_types), t_arity),
    m_param_types(std::move(t_param_types)), m_guard(std::move(t_guard)), m_parsenode(std::move(t_parsenode)) {
        // assert(t_parsenode);
    }
    ...
    bool operator==(const Proxy_Function_Base& rhs) const override {
        ...
        return this == &rhs || ((prhs != nullptr) && this->m_arity == prhs->m_arity && !this->m_guard &&
                                !prhs->m_guard && this->m_param_types == prhs->m_param_types);
    }
...
protected:
    ...
    // first result: is a match
    // second result: needs conversions
    std::pair<bool, bool> call_match_internal(const std::vector<Boxed_Value>& vals,
                                              const Type_Conversions_State& t_conversions) const {
...
public:
    Dynamic_Proxy_Function_Impl(Callable t_f, int t_arity = -1, std::shared_ptr<AST_Node> t_parsenode = AST_NodePtr(),
                                Param_Types t_param_types = Param_Types(), Proxy_Function t_guard = Proxy_Function()) :
...
protected:
    Boxed_Value do_call(const std::vector<Boxed_Value>& params,
...
public:
    ...
    virtual bool compare_types_with_cast(const std::vector<Boxed_Value>& vals,
                                         const Type_Conversions_State& t_conversions) const = 0;
};
...
public:
    ...
    Proxy_Function_Impl_Base(detail::build_param_type_list(static_cast<Func*>(nullptr))), m_f(std::move(f)) {
    }

    bool compare_types_with_cast(const std::vector<Boxed_Value>& vals,
                                 const Type_Conversions_State& t_conversions) const override {
...
public:
    explicit Assignable_Proxy_Function(const std::vector<Type_Info>& t_types) : Proxy_Function_Impl_Base(t_types) {
    }
...
public:
    Assignable_Proxy_Function_Impl(std::reference_wrapper<std::function<Func>> t_f,
...
public:
    explicit Attribute_Access(T Class::*t_attr) : Proxy_Function_Base(param_types(), 1), m_attr(t_attr) {
    }

    bool is_attribute_function() const override {
        return true;
    }
...
private:
    ...
    template <typename Type>
    auto do_call_impl(const Class* o) const -> std::enable_if_t<!std::is_pointer<Type>::value, Boxed_Value> {
        return detail::Handle_Return<
            typename std::add_lvalue_reference<typename std::add_const<Type>::type>::type>::handle(o->*m_attr);
    }
...
public:
    dispatch_error(std::vector<Boxed_Value> t_parameters, std::vector<Const_Proxy_Function> t_functions) :
    std::runtime_error("Error with function dispatch"), parameters(std::move(t_parameters)),
    ...
    parameters(std::move(t_parameters)), functions(std::move(t_functions)) {
    }
...
Boxed_Value dispatch_with_conversions(InItr begin, const InItr& end, const std::vector<Boxed_Value>& plist,
    ...
    while (begin != end) {
        if (types_match_except_for_arithmetic(begin->second, plist, t_conversions)) {
            if (matching_func == end) {
                ...
                } else if (!plist[0].is_const() && !mat_fun_param_types[1].is_const() &&
    ...
    std::transform(tis.begin() + 1, tis.end(), plist.begin(), std::back_inserter(newplist),
...
Boxed_Value dispatch(const Funcs& funcs, const std::vector<Boxed_Value>& plist,
                     const Type_Conversions_State& t_conversions) {
