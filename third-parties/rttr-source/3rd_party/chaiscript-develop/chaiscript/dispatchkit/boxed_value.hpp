...
private:
    ...
    struct Data {
        Data(const Type_Info& ti, chaiscript::detail::Any to, bool is_ref, const void* t_void_ptr,
             bool t_return_value) :
        m_type_info(ti),
        m_obj(std::move(to)), m_data_ptr(ti.is_const() ? nullptr : const_cast<void*>(t_void_ptr)),
        m_const_data_ptr(t_void_ptr), m_is_ref(is_ref), m_return_value(t_return_value) {
        }
    ...
    struct Object_Data {
        static auto get(Boxed_Value::Void_Type, bool t_return_value) {
            return std::make_shared<Data>(detail::Get_Type_Info<void>::get(), chaiscript::detail::Any(), false, nullptr,
                                          t_return_value);
        }
        ...
        template <typename T>
        static auto get(const std::shared_ptr<T>& obj, bool t_return_value) {
            return std::make_shared<Data>(detail::Get_Type_Info<T>::get(), chaiscript::detail::Any(obj), false,
        ...
        static auto get(std::shared_ptr<T>&& obj, bool t_return_value) {
            auto ptr = obj.get();
            return std::make_shared<Data>(detail::Get_Type_Info<T>::get(), chaiscript::detail::Any(std::move(obj)),
        ...
        static auto get(T t, bool t_return_value) {
            ...
            auto ptr = p.get();
            return std::make_shared<Data>(detail::Get_Type_Info<T>::get(), chaiscript::detail::Any(std::move(p)), false,
                                          ptr, t_return_value);
        }

        static std::shared_ptr<Data> get() {
            return std::make_shared<Data>(Type_Info(), chaiscript::detail::Any(), false, nullptr, false);
        }
    };
...
public:
    ...
    auto pointer_sentinel(std::shared_ptr<T>& ptr) const {
        struct Sentinel {
            Sentinel(std::shared_ptr<T>& t_ptr, Data& data) : m_ptr(t_ptr), m_data(data) {
            }
...
private:
    ...
    Boxed_Value(std::shared_ptr<Data> t_data, Internal_Construction) : m_data(std::move(t_data)) {
    }

    std::shared_ptr<Data> m_data = Object_Data::get();
};

/// @brief Creates a Boxed_Value. If the object passed in is a value type, it is copied. If it is a pointer,
/// std::shared_ptr, or std::reference_type
///        a copy is not made.
