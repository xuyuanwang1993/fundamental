...
struct Bidir_Range {
    ...
    Bidir_Range(Container& c) : m_begin(c.begin()), m_end(c.end()) {
    }
...
void input_range_type(const std::string& type, Module& m) {
    ...
    detail::input_range_type_impl<Bidir_Range<const ContainerType, typename ContainerType::const_iterator>>(
        "Const_" + type, m);
}
template <typename ContainerType>
...
void string_type(const std::string& type, Module& m) {
    ...
    m.add(fun([](const String* s, const String& f, size_t pos) { return s->find_last_not_of(f, pos); }),
          "find_last_not_of");
    m.add(fun([](const String* s, const String& f, size_t pos) { return s->find_first_not_of(f, pos); }),
          "find_first_not_of");
