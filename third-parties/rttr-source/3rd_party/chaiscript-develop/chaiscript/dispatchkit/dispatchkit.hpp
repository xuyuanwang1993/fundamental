...
/// \brief Classes and functions specific to the runtime dispatch side of ChaiScript. Some items may be of use to the
/// end user.
...
public:
    global_non_const() noexcept : std::runtime_error("a global object must be const") {
    }
...
public:
    ...
    Proxy_Function_Base(build_type_infos(t_funcs), calculate_arity(t_funcs)), m_funcs(std::move(t_funcs)) {
    }
    ...
    static std::vector<Type_Info> build_type_infos(const std::vector<Proxy_Function>& t_funcs) {
        ...
        if (begin != end) {
            ...
            assert(!type_infos.empty() &&
                   " type_info vector size is < 0, this is only possible if something else is broken");
...
public:
    ...
    explicit Dispatch_Engine(chaiscript::parser::ChaiScript_Parser_Base& parser) : m_stack_holder(), m_parser(parser) {
    }
    ...
    void add(Boxed_Value obj, const std::string& name) {
        ...
        for (auto stack_elem = stack.rbegin(); stack_elem != stack.rend(); ++stack_elem) {
            auto itr = std::find_if(stack_elem->begin(), stack_elem->end(),
                                    [&](const std::pair<std::string, Boxed_Value>& o) { return o.first == name; });
    ...
    Boxed_Value& add_get_object(const std::string& t_name, Boxed_Value obj, Stack_Holder& t_holder) {
        ...
        if (std::any_of(stack_elem.begin(), stack_elem.end(),
                        [&](const std::pair<std::string, Boxed_Value>& o) { return o.first == t_name; })) {
    ...
    Boxed_Value get_object(const std::string& name, std::atomic_uint_fast32_t& t_loc, Stack_Holder& t_holder) const {
        ...
        if (loc == 0) {
                ...
                for (auto s = stack_elem->begin(); s != stack_elem->end(); ++s) {
                    if (s->first == name) {
                        t_loc = static_cast<uint_fast32_t>(std::distance(stack.rbegin(), stack_elem) << 16) |
                                ...
                                static_cast<uint_fast32_t>(Loc::located) | static_cast<uint_fast32_t>(Loc::is_local);
                        return s->second;
                    }
                }
            }
            ...
            return stack[stack.size() - 1 - ((loc & static_cast<uint_fast32_t>(Loc::stack_mask)) >> 16)]
    ...
    std::pair<size_t, std::shared_ptr<std::vector<Proxy_Function>>> get_function(const std::string& t_name,
...
#endif
    Boxed_Value call_member(const std::string& t_name, std::atomic_uint_fast32_t& t_loc,
                            const std::vector<Boxed_Value>& params, bool t_has_params,
        ...
        const auto do_attribute_call = [this](int l_num_params, const std::vector<Boxed_Value>& l_params,
            ...
            if (l_num_params < int(l_params.size()) ||
                bv.get_type_info().bare_equal(user_type<dispatch::Proxy_Function_Base>())) {
                struct This_Foist {
        ...
        if (is_attribute_call(*funs.second, params, t_has_params, t_conversions)) {
                ...
                try {
                    if (is_no_param) {
                        ...
                        return dispatch::dispatch(
                            functions,
                            { params[0], var(t_name), var(std::vector<Boxed_Value>(params.begin() + 1, params.end())) },
                            t_conversions);
                    }
                } catch (const dispatch::option_explicit_set& e) {
                    throw chaiscript::exception::dispatch_error(
                        params, std::vector<Const_Proxy_Function>(funs.second->begin(), funs.second->end()), e.what());
                }
            }
            ...
            if (except) {
                ...
                throw chaiscript::exception::dispatch_error(
                    params, std::vector<Const_Proxy_Function>(funs.second->begin(), funs.second->end()));
            }
        }
    }
#ifdef CHAISCRIPT_MSVC
    ...
    Boxed_Value call_function(const std::string& t_name, std::atomic_uint_fast32_t& t_loc,
    ...
    void dump_function(const std::pair<const std::string, Proxy_Function>& f) const {
        ...
        for (std::vector<Type_Info>::const_iterator itr = params.begin() + 1; itr != params.end();) {
            dump_type(*itr);
            ++itr;
...
private:
    ...
    static typename Container::iterator find_keyed_value(Container& t_c, const Key& t_key) {
        return std::find_if(t_c.begin(), t_c.end(),
                            [&t_key](const typename Container::value_type& o) { return o.first == t_key; });
    }
    ...
    static typename Container::const_iterator find_keyed_value(const Container& t_c, const Key& t_key,
        ...
        if (t_c.size() > t_hint && t_c[t_hint].first == t_key) {
            return std::next(
                t_c.begin(),
                static_cast<typename std::iterator_traits<typename Container::const_iterator>::difference_type>(
                    t_hint));
        } else {
    ...
    void add_function(const Proxy_Function& t_f, const std::string& t_name) {
        ...
        Proxy_Function new_func = [&]() -> Proxy_Function {
            if (itr != funcs.end()) {
                ...
                funcs.emplace_back(t_name, std::make_shared<std::vector<Proxy_Function>>(
                                               std::initializer_list<Proxy_Function>({ t_f })));
                return t_f;
            }
        }();
...
public:
    ...
    m_engine(t_engine), m_stack_holder(t_engine.get_stack_holder()),
