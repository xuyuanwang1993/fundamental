...
class ChaiScript_Basic {
    ...
    Boxed_Value do_eval(const std::string& t_input, const std::string& t_filename = "__EVAL__",
    ...
    void build_eval_system(const ModulePtr& t_lib, const std::vector<Options>& t_opts) {
        ...
        m_engine.add(
            fun([this](const Boxed_Value& t_bv, const std::string& t_type) { return m_engine.is_type(t_bv, t_type); }),
            "is_type");
        ...
        m_engine.add(dispatch::make_dynamic_proxy_function(
                         [this](const std::vector<Boxed_Value>& t_params) { return m_engine.call_exists(t_params); }),
                     "call_exists");

        m_engine.add(fun([=](const dispatch::Proxy_Function_Base& t_fun,
        ...
        m_engine.add(fun([this](const std::string& t_type_name, bool t_throw) {
                     ...
                     "type");
        m_engine.add(fun([this](const std::string& t_type_name) { return m_engine.get_type(t_type_name, true); }),
                     "type");

        m_engine.add(fun([=](const Type_Info& t_from, const Type_Info& t_to,
        ...
        if (std::find(t_opts.begin(), t_opts.end(), Options::No_Load_Modules) == t_opts.end() &&
            ...
            m_engine.add(fun([this](const std::string& t_module, const std::string& t_file) {
                             return load_module(t_module, t_file);
                         }),
                         "load_module");
        ...
        m_engine.add(fun([this](const std::string& t_str, const bool t_dump) { return parse(t_str, t_dump); }),
                     "parse");
        m_engine.add(fun([this](const std::string& t_str) { return parse(t_str); }), "parse");

        m_engine.add(
            fun([this](const Boxed_Value& t_bv, const std::string& t_name) { add_global_const(t_bv, t_name); }),
            "add_global_const");
        m_engine.add(fun([this](const Boxed_Value& t_bv, const std::string& t_name) { add_global(t_bv, t_name); }),
                     "add_global");
        m_engine.add(fun([this](const Boxed_Value& t_bv, const std::string& t_name) { set_global(t_bv, t_name); }),
                     "set_global");

        m_engine.add(fun([this](const std::string& t_namespace_name) {
                         register_namespace([](Namespace& /*space*/) {}, t_namespace_name);
...
public:
    ...
    ChaiScript_Basic(const ModulePtr& t_lib, std::unique_ptr<parser::ChaiScript_Parser_Base>&& parser,
                     std::vector<std::string> t_module_paths = {}, std::vector<std::string> t_use_paths = {},
    ...
    m_use_paths(ensure_minimum_path_vec(std::move(t_use_paths))), m_parser(std::move(parser)), m_engine(*m_parser) {
#if !defined(CHAISCRIPT_NO_DYNLOAD) && defined(_POSIX_VERSION) && !defined(__CYGWIN__)
    ...
    Boxed_Value use(const std::string& t_filename) {
        for (const auto& path : m_use_paths) {
            try {
                ...
                chaiscript::detail::threading::unique_lock<chaiscript::detail::threading::recursive_mutex> l(
                    m_use_mutex);
    ...
    ///
    /// \throw chaiscript::exception::eval_error In the case that evaluation fails.
    /// \throw chaiscript::exception::bad_boxed_cast In the case that evaluation succeeds but the result value cannot be
    /// converted
    ...
    /// \brief Registers a namespace generator, which delays generation of the namespace until it is imported, saving
    /// memory if it is never used. \param[in] t_namespace_generator Namespace generator function. \param[in]
    /// t_namespace_name Name of the Namespace function being registered. \throw std::runtime_error In the case that the
    /// namespace name was already registered.
    void register_namespace(const std::function<void(Namespace&)>& t_namespace_generator,
        ...
        if (!m_namespace_generators.count(t_namespace_name)) {
            ...
            m_namespace_generators.emplace(
                std::make_pair(t_namespace_name, [=, space = Namespace()]() mutable -> Namespace& {
