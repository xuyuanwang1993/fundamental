...
class ChaiScript_Parser final : public ChaiScript_Parser_Base {
    ...
    static std::array<std::array<bool, detail::lengthof_alphabet>, detail::max_alphabet> build_alphabet() {
        ...
        for (size_t c = 'a'; c <= 'z'; ++c) {
            alphabet[detail::keyword_alphabet][c] = true;
        }
        for (size_t c = 'A'; c <= 'Z'; ++c) {
            alphabet[detail::keyword_alphabet][c] = true;
        }
        ...
        for (size_t c = '0'; c <= '9'; ++c) {
            alphabet[detail::int_alphabet][c] = true;
        }
        for (size_t c = '0'; c <= '9'; ++c) {
            alphabet[detail::float_alphabet][c] = true;
        }
        alphabet[detail::float_alphabet][static_cast<size_t>('.')] = true;

        for (size_t c = '0'; c <= '9'; ++c) {
            alphabet[detail::hex_alphabet][c] = true;
        }
        ...
        for (size_t c = '0'; c <= '1'; ++c) {
            alphabet[detail::bin_alphabet][c] = true;
        }
        alphabet[detail::b_alphabet][static_cast<size_t>('b')] = true;
        alphabet[detail::b_alphabet][static_cast<size_t>('B')] = true;

        for (size_t c = 'a'; c <= 'z'; ++c) {
            alphabet[detail::id_alphabet][c] = true;
        }
        for (size_t c = 'A'; c <= 'Z'; ++c) {
            alphabet[detail::id_alphabet][c] = true;
        }
    ...
    static const std::array<Operator_Precidence, 12>& create_operators() {
        static const std::array<Operator_Precidence, 12> operators = {
            { Operator_Precidence::Ternary_Cond, Operator_Precidence::Logical_Or, Operator_Precidence::Logical_And,
              Operator_Precidence::Bitwise_Or, Operator_Precidence::Bitwise_Xor, Operator_Precidence::Bitwise_And,
              Operator_Precidence::Equality, Operator_Precidence::Comparison, Operator_Precidence::Shift,
              Operator_Precidence::Addition, Operator_Precidence::Multiplication, Operator_Precidence::Prefix }
        };
        return operators;
    }
    ...
    void validate_object_name(const std::string& name) const {
        if (!Name_Validator::valid_object_name(name)) {
            throw exception::eval_error("Invalid Object Name: " + name, File_Position(m_position.line, m_position.col),
                                        *m_filename);
        }
    }
...
public:
    ...
    void build_match(size_t t_match_start, std::string t_text = "") {
        ...
        Parse_Location filepos = [&]() -> Parse_Location {
            ...
            if (t_match_start != m_match_stack.size()) {
                is_deep = true;
                return Parse_Location(m_filename, m_match_stack[t_match_start]->location.start.line,
                                      m_match_stack[t_match_start]->location.start.column, m_position.line,
                                      m_position.col);
            } else {
        ...
        m_match_stack.push_back(
            m_optimizer.optimize(chaiscript::make_unique<chaiscript::eval::AST_Node_Impl<Tracer>, NodeType>(
                std::move(t_text), std::move(filepos), std::move(new_children))));
    }
    ...
    #pragma GCC diagnostic ignored "-Wsign-compare"
        ...
        try {
            ...
            } else if (!unsigned_ && !longlong_ && u >= std::numeric_limits<long>::min() &&
                       u <= std::numeric_limits<long>::max()) {
            ...
            } else if (!unsigned_ && u >= std::numeric_limits<long long>::min() &&
                       u <= std::numeric_limits<long long>::max()) {
            ...
            try {
                ...
                if (!longlong_ && u >= std::numeric_limits<unsigned long>::min() &&
                    u <= std::numeric_limits<unsigned long>::max()) {
                    return const_var(static_cast<unsigned long>(u));
                } else {
                    return const_var(static_cast<unsigned long long>(u));
    ...
    std::unique_ptr<eval::AST_Node_Impl<Tracer>> make_node(std::string t_match, const int t_prev_line,
                                                           const int t_prev_col, Param&&... param) {
        return chaiscript::make_unique<eval::AST_Node_Impl<Tracer>, T>(
            std::move(t_match), Parse_Location(m_filename, t_prev_line, t_prev_col, m_position.line, m_position.col),
        ...
        if (m_position.has_more() && char_in_alphabet(*m_position, detail::float_alphabet)) {
            try {
                if (Hex_()) {
                    ...
                    m_match_stack.emplace_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(match), start.line,
                                                                                          start.col, std::move(bv)));
                    return true;
                }
                ...
                if (Binary_()) {
                    ...
                    m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(match), start.line,
                                                                                       start.col, std::move(bv)));
                    return true;
                }
                if (Float_()) {
    ...
    /// Reads an identifier from input which conforms to C's identifier naming conventions, without skipping initial
    /// whitespace
    bool Id_() {
    ...
    #pragma warning(disable : 4307)
            ...
            case utility::fnv1a_32("true"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(text), start.line,
                                                                                   start.col, const_var(true)));
            } break;
            case utility::fnv1a_32("false"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(text), start.line,
                                                                                   start.col, const_var(false)));
            } break;
            case utility::fnv1a_32("Infinity"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(
                    std::move(text), start.line, start.col, const_var(std::numeric_limits<double>::infinity())));
            } break;
            case utility::fnv1a_32("NaN"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(
                    std::move(text), start.line, start.col, const_var(std::numeric_limits<double>::quiet_NaN())));
            } break;
            case utility::fnv1a_32("__LINE__"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(text), start.line,
                                                                                   start.col, const_var(start.line)));
            } break;
            case utility::fnv1a_32("__FILE__"): {
                m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(std::move(text), start.line,
                                                                                   start.col, const_var(m_filename)));
            } break;
    ...
    struct Char_Parser {
        ...
        void parse(const char_type t_char, const int line, const int col, const std::string& filename) {
            ...
            const bool is_hex_char = (t_char >= '0' && t_char <= '9') || (t_char >= 'a' && t_char <= 'f') ||
                                     (t_char >= 'A' && t_char <= 'F');
    ...
    bool Quoted_String() {
        ...
        if (Quoted_String_()) {
                ...
                while (s != end) {
                    if (cparser.saw_interpolation_marker) {
                        if (*s == '{') {
                            ...
                            m_match_stack.push_back(make_node<eval::Constant_AST_Node<Tracer>>(
                                match, start.line, start.col, const_var(match)));
                            ...
                            if (*s == '}') {
                                ...
                                m_match_stack.push_back(
                                    make_node<eval::Id_AST_Node<Tracer>>("to_string", start.line, start.col));
                                ...
                                throw exception::eval_error("Unclosed in-string eval",
                                                            File_Position(start.line, start.col), *m_filename);
                            }
                        } else {
                            match.push_back('$');
                        }
            ...
            m_match_stack.push_back(
                make_node<eval::Constant_AST_Node<Tracer>>(match, start.line, start.col, const_var(match)));
    ...
    bool Single_Quoted_String() {
        ...
        if (Single_Quoted_String_()) {
            ...
            m_match_stack.push_back(
                make_node<eval::Constant_AST_Node<Tracer>>(match, start.line, start.col, const_var(char(match.at(0)))));
            return true;
        } else {
            return false;
        }
        ...
        return std::any_of(m_operator_matches.begin(), m_operator_matches.end(),
                           [t_s](const std::vector<utility::Static_String>& opers) {
                               return std::any_of(opers.begin(), opers.end(),
                                                  [t_s](const utility::Static_String& s) { return t_s == s.c_str(); });
                           });
    }
    ...
    bool Symbol(const utility::Static_String& t_s, const bool t_disallow_prevention = false) {
        ...
        // ignore substring matches
        if (retval && m_position.has_more() && (t_disallow_prevention == false) &&
            char_in_alphabet(*m_position, detail::symbol_alphabet)) {
            if (*m_position != '=' && is_operator(Position::str(start, m_position)) &&
                !is_operator(Position::str(start, m_position + 1))) {
    ...
    bool Container_Arg_List() {
        ...
        if (Value_Range()) {
            ...
            while (Char(',')) {
                ...
                if (!Map_Pair()) {
                    throw exception::eval_error("Unexpected value in container",
                                                File_Position(m_position.line, m_position.col), *m_filename);
                }
            }
            build_match<eval::Arg_List_AST_Node<Tracer>>(prev_stack_top);
    ...
    bool Def(const bool t_class_context = false, const std::string& t_class_name = "") {
        ...
        if (Keyword("def")) {
            ...
            if (t_class_context) {
                m_match_stack.push_back(
                    make_node<eval::Id_AST_Node<Tracer>>(t_class_name, m_position.line, m_position.col));
            }
    ...
    bool Try() {
        ...
        if (Keyword("try")) {
            ...
            while (has_matches) {
                ...
                if (Keyword("catch")) {
                    ...
                    if (Char('(')) {
                        ...
                        if (Char(':')) {
                            if (!Operator()) {
                                throw exception::eval_error("Missing guard expression for catch",
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'catch' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            if (Keyword("finally")) {
                ...
                if (!Block()) {
                    throw exception::eval_error("Incomplete 'finally' block",
                                                File_Position(m_position.line, m_position.col), *m_filename);
                }
    ...
    bool If() {
        ...
        if (Keyword("if")) {
            ...
            while (has_matches) {
                ...
                if (Keyword("else")) {
                    if (If()) {
                        ...
                        if (!Block()) {
                            throw exception::eval_error("Incomplete 'else' block",
                                                        File_Position(m_position.line, m_position.col), *m_filename);
                        }
                        has_matches = true;
                    }
            ...
            if ((is_if_init && num_children == 3) || (!is_if_init && num_children == 2)) {
                m_match_stack.push_back(
                    chaiscript::make_unique<eval::AST_Node_Impl<Tracer>, eval::Noop_AST_Node<Tracer>>());
            }
    ...
    bool Class(const bool t_class_allowed) {
        ...
        if (Keyword("class")) {
            ...
            if (!Class_Block(class_name)) {
                throw exception::eval_error("Incomplete 'class' block", File_Position(m_position.line, m_position.col),
                                            *m_filename);
            }
    ...
    bool For_Guards() {
        ...
        if (!(Equation() && Eol())) {
            if (!Eol()) {
                ...
                m_match_stack.push_back(
                    chaiscript::make_unique<eval::AST_Node_Impl<Tracer>, eval::Constant_AST_Node<Tracer>>(
                        Boxed_Value(true)));
            }
        }
    ...
    bool Case() {
        ...
        if (Keyword("case")) {
            ...
            if (!Char('(')) {
                throw exception::eval_error("Incomplete 'case' expression",
                                            File_Position(m_position.line, m_position.col), *m_filename);
            }
            ...
            if (!Block()) {
                throw exception::eval_error("Incomplete 'default' block",
                                            File_Position(m_position.line, m_position.col), *m_filename);
            }
    ...
    bool Dot_Fun_Array() {
        ...
        const auto prev_stack_top = m_match_stack.size();
        if (Lambda() || Num() || Quoted_String() || Single_Quoted_String() || Paren_Expression() ||
            Inline_Container() || Id(false)) {
            retval        = true;
            bool has_more = true;
                ...
                if (Char('(')) {
                    ...
                    if (!m_match_stack.back()->children.empty()) {
                        if (m_match_stack.back()->children[0]->identifier == AST_Node_Type::Dot_Access) {
                            ...
                            func_call->children.insert(func_call->children.begin(),
                                                       std::move(dot_access->children.back()));
                    ...
                    if (std::distance(m_match_stack.begin() + static_cast<int>(prev_stack_top), m_match_stack.end()) !=
                        2) {
                        throw exception::eval_error("Incomplete dot access fun call",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
    ...
    bool Var_Decl(const bool t_class_context = false, const std::string& t_class_name = "") {
        ...
        if (t_class_context && (Keyword("attr") || Keyword("auto") || Keyword("var"))) {
            ...
            if (!Id(true)) {
                throw exception::eval_error("Incomplete attribute declaration",
                                            File_Position(m_position.line, m_position.col), *m_filename);
            }
            ...
            if (!Id(true)) {
                throw exception::eval_error("Missing attribute name in definition",
                                            File_Position(m_position.line, m_position.col), *m_filename);
            }
    ...
    bool Prefix() {
        ...
        constexpr const std::array<utility::Static_String, 6> prefix_opers { { SS { "++" }, SS { "--" }, SS { "-" },
                                                                               SS { "+" }, SS { "!" }, SS { "~" } } };
    ...
    bool Map_Pair() {
        ...
        if (Operator()) {
            if (Symbol(":")) {
                retval = true;
                if (!Operator()) {
                    throw exception::eval_error("Incomplete map pair", File_Position(m_position.line, m_position.col),
                                                *m_filename);
                }
    ...
    bool Value_Range() {
        ...
        if (Operator()) {
            if (Symbol("..")) {
                retval = true;
                if (!Operator()) {
                    throw exception::eval_error("Incomplete value range",
                                                File_Position(m_position.line, m_position.col), *m_filename);
                }
    ...
    bool Equation() {
        ...
        if (Operator()) {
            for (const auto& sym : { SS { "=" }, SS { ":=" }, SS { "+=" }, SS { "-=" }, SS { "*=" }, SS { "/=" },
                                     SS { "%=" }, SS { "<<=" }, SS { ">>=" }, SS { "&=" }, SS { "^=" }, SS { "|=" } }) {
                if (Symbol(sym, true)) {
                    SkipWS(true);
                    if (!Equation()) {
