...
struct IRunner {
    ...
    // The double negation silences MSVC's C4800 warning, the static_cast forces short-circuit evaluation if the type
    // has overloaded &&.
...
struct ContainsMatcher : MatcherBase<std::vector<T>, std::vector<T>> {
    ...
    bool match(std::vector<T> const& v) const CATCH_OVERRIDE {
        ...
        for (size_t i = 0; i < m_comparator.size(); ++i)
            if (std::find(v.begin(), v.end(), m_comparator[i]) == v.end()) return false;
        return true;
    }
    virtual std::string describe() const CATCH_OVERRIDE {
        return "Contains: " + Catch::toString(m_comparator);
...
struct ConfigData {
    ...
    ConfigData() :
    listTests(false), listTags(false), listReporters(false), listTestNamesOnly(false), listExtraInfo(false),
    showSuccessfulTests(false), shouldDebugBreak(false), noThrow(false), showHelp(false), showInvisibles(false),
    filenamesAsTags(false), libIdentify(false), abortAfter(-1), rngSeed(0), verbosity(Verbosity::Normal),
    warnings(WarnAbout::Nothing), showDurations(ShowDurations::DefaultForReporter),
    runOrder(RunTests::InDeclarationOrder), useColour(UseColour::Auto), waitForKeypress(WaitForKeypress::Never) {
    }
...
void convertInto(std::string const& _source, T& _dest) {
    ...
    ss << _source;
    ss >> _dest;
    if (ss.fail()) throw std::runtime_error("Unable to convert " + _source + " to destination type");
}
...
public:
    ...
    Mode handlePositional(std::size_t i, char c, std::string const& arg, std::vector<Token>& tokens) {
        if (inQuotes || std::string("\0", 1).find(c) == std::string::npos) return mode;
...
public:
    ...
    m_boundProcessName(other.m_boundProcessName), m_options(other.m_options), m_positionalArgs(other.m_positionalArgs),
...
inline void setRngSeed(ConfigData& config, std::string const& seed) {
    if (seed == "time") {
        ...
        ss << seed;
        ss >> config.rngSeed;
        if (ss.fail()) throw std::runtime_error("Argument to --rng-seed should be the word 'time' or a number");
    }
}
...
public:
    Text(std::string const& _str, TextAttributes const& _attr = TextAttributes()) : attr(_attr) {
        ...
        while (it != strEnd) {
            ...
            if (indent.size() != _attr.indent) indent = std::string(_attr.indent, ' ');
            it = itNext;
        }
    }
...
struct SectionStats {
    ...
    assertions(_assertions), durationInSeconds(_durationInSeconds), missingAssertions(_missingAssertions) {
    }
...
inline std::size_t listTests(Config const& config) {
    ...
    for (std::vector<TestCase>::const_iterator it = matchedTestCases.begin(), itEnd = matchedTestCases.end();
        ...
        if (!testCaseInfo.tags.empty()) Catch::cout() << Text(testCaseInfo.tagsAsString, tagsAttr) << std::endl;
    }
...
inline std::size_t listTestsNamesOnly(Config const& config) {
    TestSpec testSpec = config.testSpec();
    if (!config.testSpec().hasFilters()) testSpec = TestSpecParser(ITagAliasRegistry::get()).parse("*").testSpec();
    std::size_t matchedTests               = 0;
    std::vector<TestCase> matchedTestCases = filterTests(getAllTestCasesSorted(config), testSpec, config);
    for (std::vector<TestCase>::const_iterator it = matchedTestCases.begin(), itEnd = matchedTestCases.end();
         it != itEnd; ++it) {
        ...
        if (startsWith(testCaseInfo.name, '#'))
            ...
            Catch::cout() << testCaseInfo.name;
        if (config.listExtraInfo()) Catch::cout() << "\t@" << testCaseInfo.lineInfo;
        Catch::cout() << std::endl;
    }
    return matchedTests;
}
...
public:
    ...
    static IndexTracker& acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation, int size) {
        ...
        if (!ctx.completedCycle() && !tracker->isComplete()) {
            if (tracker->m_runState != ExecutingChildren && tracker->m_runState != NeedsAnotherRun) tracker->moveNext();
            tracker->open();
        }
...
public:
    ...
    m_runInfo(_config->name()), m_context(getCurrentMutableContext()), m_activeTestCase(CATCH_NULL), m_config(_config),
    m_reporter(reporter), m_shouldReportUnexpected(true) {
...
Ptr<IStreamingReporter> makeReporter(Ptr<Config> const& config) {
    ...
    if (reporters.empty()) reporters.push_back(CATCH_CONFIG_DEFAULT_REPORTER);
...
IColourImpl* platformColourInstance() {
    ...
    if (colourMode == UseColour::Auto) colourMode = !isDebuggerActive() ? UseColour::Yes : UseColour::No;
...
lineInfo(_lineInfo), capturedExpression(_capturedExpression), resultDisposition(_resultDisposition),
...
TestCaseInfo::TestCaseInfo(std::string const& _name, std::string const& _className, std::string const& _description,
                           std::set<std::string> const& _tags, SourceLineInfo const& _lineInfo) :
name(_name),
className(_className), description(_description), lineInfo(_lineInfo), properties(None) {
    setTags(*this, _tags);
}

TestCaseInfo::TestCaseInfo(TestCaseInfo const& other) :
name(other.name), className(other.className), description(other.description), tags(other.tags),
lcaseTags(other.lcaseTags), tagsAsString(other.tagsAsString), lineInfo(other.lineInfo), properties(other.properties) {
}
...
minorVersion(_minorVersion), patchNumber(_patchNumber), branchName(_branchName), buildNumber(_buildNumber) {
}
...
void LegacyReporterAdapter::sectionEnded(SectionStats const& sectionStats) {
    if (sectionStats.missingAssertions) m_legacyReporter->NoAssertionsInSection(sectionStats.sectionInfo.name);
...
std::string toString(char value) {
    ...
    if (value == '\n') return "'\\n'";
    if (value == '\t') return "'\\t'";
    if ('\0' <= value && value < ' ') return toString(static_cast<unsigned int>(value));
    char chstr[] = "' '";
    chstr[1]     = value;
    return chstr;
}
...
m_shouldDebugBreak(false), m_shouldThrow(false), m_guardException(false), m_usedStream(false) {
}
...
AssertionResult ResultBuilder::build(DecomposedExpression const& expr) const {
    ...
    if (m_usedStream) data.message = m_stream().oss.str();
...
public:
    ...
    CumulativeReporterBase(_config), xml(_config.stream()), unexpectedExceptions(0), m_okToFail(false) {
        m_reporterPrefs.shouldRedirectStdOut = true;
    }
