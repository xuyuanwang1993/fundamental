...
TEST_CASE("EventQueue, multi threading, int, void (int)") {
    ...
    for (int i = 0; i < itemCount; ++i) {
        queue.appendListener(eventList[i], [&queue, i, &dataList](const int d) { dataList[i] += d; });
    }
...
TEST_CASE("EventQueue, multi threading, one thread waits") {
    ...
    std::thread thread([stopEvent, otherEvent, &dataList, &queue, &threadProcessCount]() {
        ...
        queue.appendListener(otherEvent, [&dataList](const int index) { dataList[index] += index + 1; });
...
TEST_CASE("EventQueue, multi threading, many threads wait") {
    ...
    queue.appendListener(stopEvent, [&shouldStop](int) { shouldStop = true; });
    queue.appendListener(otherEvent, [&dataList](const int index) { ++dataList[index]; });
