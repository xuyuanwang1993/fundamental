...
TEST_CASE("EventQueue, std::string, void (const std::string &)") {
    ...
    queue.appendListener("event1", [&a](const std::string&) { a = 2; });
    queue.appendListener("event1", eraseArgs1([&b]() { b = 8; }));
...
TEST_CASE("EventQueue, int, void ()") {
    ...
    queue.appendListener(3, [&a]() { a += 1; });
    queue.appendListener(3, [&b]() { b += 3; });
...
TEST_CASE("EventQueue, customized event") {
    ...
    queue.appendListener(
        3, [&a](const MyEvent& e, const std::string& s) { a += e.message + s + std::to_string(e.param); });
    queue.appendListener(
        3, [&b](const MyEvent& e, const std::string& s) { b += e.message + s + std::to_string(e.param); });
...
TEST_CASE("EventQueue, no memory leak or double free in queued arguments") {
    struct Item {
        Item(const int index, std::vector<int>* counterList) : index(index), counterList(counterList) {
            ++(*counterList)[index];
        }
        ...
        Item(Item&& other) : index(other.index), counterList(other.counterList) {
            ++(*counterList)[index];
        }

        Item(const Item& other) : index(other.index), counterList(other.counterList) {
            ++(*counterList)[index];
        }
    ...
    auto add = [&counterList, &queue](int n) { queue->enqueue(n, Item(n, &counterList)); };
...
TEST_CASE("EventQueue, non-copyable but movable unique_ptr") {
    ...
    queue.appendListener(3, [&dataList](const PTR& ptr) { ++dataList[*ptr]; });
...
TEST_CASE("EventQueue, peekEvent/takeEvent/dispatch") {
    ...
    queue->appendListener(3, [&dataList](const SP& sp) { ++dataList[*sp]; });
...
TEST_CASE("EventQueue, processIf") {
    ...
    queue.appendListener(5, [&dataList](int) { ++dataList[0]; });
    queue.appendListener(6, [&dataList](int) { ++dataList[1]; });
    queue.appendListener(7, [&dataList](int) { ++dataList[2]; });
...
TEST_CASE("EventQueue, processIf, Predictor has no arguments, listeners has no arguments") {
    ...
    queue.appendListener(5, [&dataList]() { ++dataList[0]; });
    queue.appendListener(6, [&dataList]() { ++dataList[1]; });
    queue.appendListener(7, [&dataList]() { ++dataList[2]; });
