...
TEST_CASE("CallbackList, nested callbacks, new callbacks should not be triggered") {
    ...
    callbackList.append([&callbackList, &a, &b]() {
        ...
        auto h1 = callbackList.append([&callbackList, &b] {
            ++b;
            callbackList.append([&callbackList, &b] { ++b; });
            auto h2 = callbackList.prepend([&callbackList, &b] {
                ++b;
                callbackList.append([&callbackList, &b] { ++b; });
            });
            callbackList.append([&callbackList, &b] { ++b; });
            callbackList.insert([&callbackList, &b] { ++b; }, h2);
            callbackList.prepend([&callbackList, &b] { ++b; });
        });
        callbackList.prepend([&callbackList, &b] { ++b; });
        callbackList.insert([&callbackList, &b] { ++b; }, h1);
    });
...
TEST_CASE("CallbackList, forEachIf") {
    ...
    callbackList.append([&dataList]() { dataList[0] += 1; });
    callbackList.append([&dataList]() { dataList[1] += 2; });
    callbackList.append([&dataList]() { dataList[2] += 3; });
...
TEST_CASE("CallbackList, forEach and forEachIf") {
    ...
    for (int i = 0; i < itemCount; ++i) {
        callbackList.append([&dataList, i]() { dataList[i] = i + 1; });
    }

    SECTION("forEach") {
        callbackList.forEach([](const CL::Callback& callback) -> void { callback(); });
...
TEST_CASE("CallbackList, non-lvaue-reference arguments should not be modified by callbacks") {
    ...
    callbackList.append([&dataList](const std::string& s) { dataList[1] = s; });
