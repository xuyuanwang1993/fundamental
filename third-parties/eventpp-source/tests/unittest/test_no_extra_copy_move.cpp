...
public:
    CopyMoveCounter() : counter(std::make_shared<Counter>()) {
    }

    CopyMoveCounter(const CopyMoveCounter& other) : counter(other.counter) {
        ++counter->copied;
    }

    CopyMoveCounter(CopyMoveCounter&& other) : counter(std::move(other.counter)) {
        ++counter->moved;
    }
