...
TEST_CASE("HeterEventDispatcher, appendListener") {
    ...
    dispatcher.appendListener(event, [&orderList, &order]() { orderList[order++] = 1; });
    dispatcher.appendListener(event, [&orderList, &order]() { orderList[order++] = 2; });
    dispatcher.appendListener(event, [&orderList, &order](int) { orderList[order++] = 3; });
    dispatcher.appendListener(event, [&orderList, &order](int) { orderList[order++] = 4; });
    dispatcher.appendListener(event, [&orderList, &order](int) { orderList[order++] = 5; });
...
TEST_CASE("HeterEventDispatcher, prependListener") {
    ...
    dispatcher.prependListener(event, [&orderList, &order]() { orderList[order++] = 1; });
    dispatcher.prependListener(event, [&orderList, &order]() { orderList[order++] = 2; });
    dispatcher.prependListener(event, [&orderList, &order](int) { orderList[order++] = 3; });
    dispatcher.prependListener(event, [&orderList, &order](int) { orderList[order++] = 4; });
    dispatcher.prependListener(event, [&orderList, &order](int) { orderList[order++] = 5; });
...
TEST_CASE("HeterEventDispatcher, insertListener") {
    ...
    auto h1 = dispatcher.appendListener(event, [&orderList, &order]() { orderList[order++] = 1; });
    dispatcher.insertListener(
        event, [&orderList, &order]() { orderList[order++] = 2; }, h1);

    auto h2 = dispatcher.appendListener(event, [&orderList, &order](int) { orderList[order++] = 3; });
    dispatcher.insertListener(
        event, [&orderList, &order](int) { orderList[order++] = 4; }, h2);
    dispatcher.insertListener(
        event, [&orderList, &order](int) { orderList[order++] = 5; }, h2);
...
TEST_CASE("HeterEventDispatcher, removeListener") {
    ...
    auto h1 = dispatcher.appendListener(event, [&dataList]() { ++dataList[0]; });
    auto h2 = dispatcher.appendListener(event, [&dataList]() { ++dataList[1]; });
    auto h3 = dispatcher.appendListener(event, [&dataList](int) { ++dataList[2]; });
    auto h4 = dispatcher.appendListener(event, [&dataList](int) { ++dataList[3]; });
    auto h5 = dispatcher.appendListener(event, [&dataList](int) { ++dataList[4]; });
...
TEST_CASE("HeterEventDispatcher, forEachIf") {
    ...
    dispatcher.appendListener(event, [&dataListNoArg]() { dataListNoArg[0] += 1; });
    dispatcher.appendListener(event, [&dataListNoArg]() { dataListNoArg[1] += 2; });
    dispatcher.appendListener(event, [&dataListNoArg]() { dataListNoArg[2] += 3; });

    dispatcher.appendListener(event, [&dataListWithArg](int n) { dataListWithArg[0] += n; });
    dispatcher.appendListener(event, [&dataListWithArg](int n) { dataListWithArg[1] += n * 2; });
    dispatcher.appendListener(event, [&dataListWithArg](int n) { dataListWithArg[2] += n * 3; });
...
TEST_CASE("HeterEventDispatcher, invoke, ArgumentPassingIncludeEvent") {
    ...
    dispatcher.appendListener(event, [&dataListNoArg](int) { dataListNoArg[0] += 1; });
    dispatcher.appendListener(event, [&dataListNoArg](int) { dataListNoArg[1] += 2; });
    dispatcher.appendListener(event, [&dataListNoArg](int) { dataListNoArg[2] += 3; });

    dispatcher.appendListener(event, [&dataListWithArg](int, int n) { dataListWithArg[0] += n; });
    dispatcher.appendListener(event, [&dataListWithArg](int, int n) { dataListWithArg[1] += n * 2; });
    dispatcher.appendListener(event, [&dataListWithArg](int, int n) { dataListWithArg[2] += n * 3; });
...
TEST_CASE("HeterEventDispatcher, getEvent") {
    ...
    dispatcher.appendListener(event, [&dataList](int i) { dataList[1] = i; });
...
TEST_CASE("HeterEventDispatcher, getEvent, ArgumentPassingIncludeEvent") {
    ...
    eventpp::HeterEventDispatcher<int, eventpp::HeterTuple<void(const MyEvent&), void(const MyEvent&, int)>,
                                  EventPolicies>
        dispatcher;
    constexpr int event = 3;
    ...
    dispatcher.appendListener(event, [&dataList](const MyEvent& e) { dataList[0] = e.param; });
    dispatcher.appendListener(event, [&dataList](const MyEvent& e, int i) { dataList[1] = e.param + i; });
...
TEST_CASE("HeterEventDispatcher, event filter") {
    ...
    for (int i = 0; i < itemCount; ++i) {
        dispatcher.appendListener(i, [&dataList, i](int e, int index) { dataList[e] = index; });
    }
