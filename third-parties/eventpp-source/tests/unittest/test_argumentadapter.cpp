...
TEST_CASE("ArgumentAdapter, check binary code length")
{
    ...
    // The code between the three __debugbreak in each code block should be same length, unless there are special
comments.
...
}); auto adapted = eventpp::argumentAdapter<void(std::shared_ptr<Derived>)>(func); std::shared_ptr<Base>
