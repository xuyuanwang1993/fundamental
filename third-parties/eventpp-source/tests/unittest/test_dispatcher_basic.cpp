...
TEST_CASE("EventDispatcher, std::string, void (const std::string &)") {
    ...
    dispatcher.appendListener("event1", [&a](const std::string&) { a = 2; });
    dispatcher.appendListener("event1", eraseArgs1([&b]() { b = 8; }));
...
TEST_CASE("EventDispatcher, int, void ()") {
    ...
    dispatcher.appendListener(3, [&a]() { a = 2; });
    dispatcher.appendListener(3, [&b]() { b = 8; });
...
TEST_CASE("EventDispatcher, add/remove, int, void ()") {
    ...
    hb = dispatcher.appendListener(event, [&b]() { b = 8; });
...
TEST_CASE("EventDispatcher, add another listener inside a listener, int, void ()") {
    ...
    dispatcher.appendListener(event, [&event, &a, &dispatcher, &b]() {
        a = 2;
        dispatcher.appendListener(event, [&event, &b]() { b = 8; });
    });
...
TEST_CASE("EventDispatcher, forEach") {
    ...
    dispatcher.appendListener(event, [event, &dataList]() { ++dataList[0]; });
    dispatcher.appendListener(event, [event, &dataList]() { ++dataList[1]; });
    dispatcher.appendListener(event, [event, &dataList]() { ++dataList[2]; });
...
TEST_CASE("EventDispatcher, Event struct, void (const std::string &, int)") {
    ...
    dispatcher.appendListener(event,
                              [event, &dispatcher, &sList, &iList](const MyEvent& e, const std::string& s, int i) {
                                  sList[0] = e.message + " " + s;
                                  iList[0] = e.param + i;
                              });
    dispatcher.appendListener(event,
                              [event, &dispatcher, &sList, &iList](const MyEvent& e, std::string s, const int i) {
                                  sList[1] = s + " " + e.message;
                                  iList[1] = e.param * i;
                              });
...
TEST_CASE("EventDispatcher, many, int, void (int)") {
    ...
    for (int i = 1; i < eventCount; i += 2) {
        dispatcher.appendListener(eventList[i], [&dispatcher, i, &dataList](const int e) { dataList[i] = e; });
    }
...
TEST_CASE("EventDispatcher, explicit single threading, int, void (int)") {
    ...
    dispatcher.appendListener(3, [&a](int n) { a += n; });
    dispatcher.appendListener(3, [&b](const int& n) { b += n; });
