...
TEST_CASE("HeterCallbackList tutorial 1, basic") {
    ...
    callbackList.append([]() { std::cout << "Got callback for void()." << std::endl; });
    callbackList.append([](int) { std::cout << "Got callback for void(int)." << std::endl; });
...
TEST_CASE("HeterCallbackList tutorial 2, for each") {
    ...
    // Add some callbacks.
    callbackList.append([]() { std::cout << "Got callback void()." << std::endl; });
    callbackList.append([]() { std::cout << "Got callback void() again." << std::endl; });
    callbackList.append([](int /*a*/) { std::cout << "Got callback void(int a)." << std::endl; });
    callbackList.append([](int /*a*/) { std::cout << "Got callback void(int a) again." << std::endl; });
    ...
    // The forEach callback prototype is void(const HeterCallbackList::Handle & handle, const std::function<prototype> &
    // callback)
    int index = 0;
    callbackList.forEach<void()>(
        [&callbackList, &index](const CL::Handle& handle, const std::function<void()>& /*callback*/) {
