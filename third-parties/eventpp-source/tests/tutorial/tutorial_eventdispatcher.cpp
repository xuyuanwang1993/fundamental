...
TEST_CASE("EventDispatcher tutorial 1, basic") {
    ...
    dispatcher.appendListener(3, []() { std::cout << "Got event 3." << std::endl; });
    dispatcher.appendListener(5, []() { std::cout << "Got event 5." << std::endl; });
    dispatcher.appendListener(5, []() { std::cout << "Got another event 5." << std::endl; });
...
TEST_CASE("EventDispatcher tutorial 3, customized Event struct") {
    ...
    eventpp::EventDispatcher<int, void(const MyEvent&, bool), MyEventPolicies> dispatcher;

    // Add a listener.
    // Note: the first argument is the event type of type int, not MyEvent.
    dispatcher.appendListener(3, [](const MyEvent& e, bool b) {
        std::cout << std::boolalpha << "Got event 3" << std::endl
...
struct Tutor4MyEvent {
    ...
    explicit Tutor4MyEvent(const int type) : type(type), canceled(false) {
    }
...
TEST_CASE("EventDispatcher tutorial 4, event canceling") {
    ...
    dispatcher.appendListener(
        3, [](const Tutor4MyEvent& /*e*/) { std::cout << "Should not get this event 3" << std::endl; });
...
TEST_CASE("EventDispatcher tutorial 5, event filter") {
    ...
    dispatcher.appendListener(3, [](const int /*e*/, const int i, const std::string& s) {
        std::cout << "Got event 3, i was 1 but actual is " << i << " s was Hello but actual is " << s << std::endl;
    });
