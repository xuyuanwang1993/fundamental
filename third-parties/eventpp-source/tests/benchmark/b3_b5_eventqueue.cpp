...
void doExecuteEventQueue(const std::string& message, const size_t queueSize, const size_t iterateCount,
                         const size_t eventCount, size_t listenerCount = 0) {
    ...
    const uint64_t time = measureElapsedTime([queueSize, iterateCount, eventCount, listenerCount, &eventQueue] {
    ...
    std::cout << message << " queueSize: " << queueSize << " iterateCount: " << iterateCount
              << " eventCount: " << eventCount << " listenerCount: " << listenerCount << " Time: " << time << std::endl;
    ;
}

template <typename Policies>
void doMultiThreadingExecuteEventQueue(const std::string& message, const size_t enqueueThreadCount,
                                       const size_t processThreadCount, const size_t totalEventCount,
                                       const size_t eventCount, size_t listenerCount = 0) {
    using EQ = eventpp::EventQueue<size_t, void(size_t), Policies>;
    EQ eventQueue;
    ...
    for (size_t i = 0; i < enqueueThreadCount; ++i) {
        ...
        const size_t end =
            (i == enqueueThreadCount - 1 ? totalEventCount : start + totalEventCount / enqueueThreadCount);
    ...
    const uint64_t time = measureElapsedTime([&start, &stop, &enqueueThreadList, &processThreadList, totalEventCount,
                                              eventCount, listenerCount, &eventQueue] {
        start.store(true);
    ...
    std::cout << message << " enqueueThreadCount: " << enqueueThreadCount
              << " processThreadCount: " << processThreadCount << " totalEventCount: " << totalEventCount
              << " eventCount: " << eventCount << " listenerCount: " << listenerCount << " Time: " << time << std::endl;
    ;
}
