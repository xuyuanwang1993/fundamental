...
void doCallbackListVsFunctionList(const std::string& message, AddCL&& addCl, AddFL&& addFL) {
    ...
    const uint64_t timeCallbackList = measureElapsedTime([iterateCount, &callbackList]() {
        for (int iterate = 0; iterate < iterateCount; ++iterate) {
            callbackList(iterate, iterate);
        }
    });
    const uint64_t timeFunctionList = measureElapsedTime([iterateCount, &functionList]() {
        for (int iterate = 0; iterate < iterateCount; ++iterate) {
...
TEST_CASE("b7, CallbackList vs vector of functions") {
    ...
    std::vector<BenchmarkItem> itemList {
        { "global", [](CLT<PoliciesMultiThreading>& cl) { cl.append(&globalFunction); },
          [](CLT<PoliciesSingleThreading>& cl) { cl.append(&globalFunction); },
          [](FLT& fl) { fl.push_back(&globalFunction); } },

        { "nonInlineGlobalFunction", [](CLT<PoliciesMultiThreading>& cl) { cl.append(&nonInlineGlobalFunction); },
          [](CLT<PoliciesSingleThreading>& cl) { cl.append(&nonInlineGlobalFunction); },
          [](FLT& fl) { fl.push_back(&nonInlineGlobalFunction); } },

        { "FunctionObject", [](CLT<PoliciesMultiThreading>& cl) { cl.append(FunctionObject()); },
          [](CLT<PoliciesSingleThreading>& cl) { cl.append(FunctionObject()); },
          [](FLT& fl) { fl.push_back(FunctionObject()); } },

    };

    for (BenchmarkItem& item : itemList) {
        doCallbackListVsFunctionList<PoliciesMultiThreading>("Multi thread, " + item.message, item.addClMulti,
                                                             item.addFl);
    }
}
