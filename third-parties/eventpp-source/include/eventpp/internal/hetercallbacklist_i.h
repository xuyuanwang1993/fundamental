...
struct FindPrototypeByCallableFromIndex<N, HeterTuple<RT(Args...), Others...>, Callable, ArgTransformer, M,
    ...
    enum {
        canInvoke = CanInvoke < Callable,
        typename ArgTransformer<Args>::type... > ::value
    };
    ...
    enum {
        index = canInvoke ? N : FindPrototypeByCallableFromIndex < N + 1,
        HeterTuple<Others...>,
        Callable,
        ArgTransformer,
        M > ::index
...
struct FindPrototypeByArgsFromIndex<N, HeterTuple<RT(Args...), Others...>, InArgs...> {
    ...
    enum {
        index = canInvoke ? N : FindPrototypeByArgsFromIndex < N + 1,
        HeterTuple<Others...>,
        InArgs... > ::index
    };
};
...
struct GetCallablePrototypeMaxSize<HeterTuple<RT(Args...), Others...>, Record> {
    enum {
        ...
        other = GetCallablePrototypeMaxSize < HeterTuple<Others...>,
        Record > ::value
    };
    enum {
        value = (my > other ? (int)my : (int)other)
    };
};
...
struct GetCallablePrototypeMaxSize<HeterTuple<>, Record> {
    enum {
        value = 1
    }; // set minimum size to 1 instead of 0
};
